...
static ID_INLINE int Q_FloatToInt( float f ) {
	union { float f; int i; } u;
	u.f = f;
	return u.i;
}
#define PASSFLOAT(x) Q_FloatToInt((x))
...
#define	MAX_MODELS			330		// these are sent over the net as 8 bits (Gordon: upped to 9 bits, erm actually it was already at 9 bits, wtf? NEVAR TRUST GAMECODE COMMENTS, comments are evil :E, lets hope it doesnt horribly break anything....)
