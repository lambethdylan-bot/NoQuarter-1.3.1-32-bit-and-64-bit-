 ...
 * name:	bg_animation.c
 *
 * desc:	Incorporates several elements related to the new flexible animation system.
 *
...
int BG_PlayAnim( playerState_t *ps, animModelInfo_t *animModelInfo, int animNum, animBodyPart_t bodyPart, int forceDuration, qboolean setTimer, qboolean isContinue, qboolean force ) {
	...
	if( forceDuration ) {
		...
		duration = animModelInfo->animations[animNum]->duration + SERVER_FRAMETIME;	// account for lerping between anims
	}

	switch (bodyPart) {
		case ANIM_BP_BOTH:
		case ANIM_BP_LEGS:
			if( (ps->legsTimer < SERVER_FRAMETIME) || force ) {
		...
		case ANIM_BP_TORSO:
			if( (ps->torsoTimer < SERVER_FRAMETIME) || force ) {
...
int BG_ExecuteCommand( playerState_t *ps, animModelInfo_t *animModelInfo, animScriptCommand_t *scriptCommand, qboolean setTimer, qboolean isContinue, qboolean force ) {
	...
	if (scriptCommand->bodyPart[0]) {
		duration = scriptCommand->animDuration[0] + SERVER_FRAMETIME;
...
int BG_AnimScriptAnimation( playerState_t *ps, animModelInfo_t *animModelInfo, scriptAnimMoveTypes_t movetype, qboolean isContinue ) {
	...
	if (lastClient != ps->clientNum || lastMT != movetype) {
		Com_Printf( "script anim: cl %i, mt %s\n", ps->clientNum, animMoveTypesStr[movetype] );
...
#ifdef DBGANIMS
	if( scriptCommand->bodyPart[0] ) {
		Com_Printf( "anim0 (%s): %s %d\n", animBodyPartsStr[scriptCommand->bodyPart[0]].string, animModelInfo->animations[scriptCommand->animIndex[0]]->name, scriptCommand->animIndex[0] );
	}
	if( scriptCommand->bodyPart[1] ) {
		Com_Printf( "anim1 (%s): %s %d\n", animBodyPartsStr[scriptCommand->bodyPart[1]].string, animModelInfo->animations[scriptCommand->animIndex[1]]->name, scriptCommand->animIndex[1] );
	}
	Com_Printf( "\n" );
#endif
...
#ifdef DBGANIMEVENTS
	Com_Printf( "script event: cl %i, ev %s\n", ps->clientNum, animEventTypesStr[event] );
#endif
...
#ifdef DBGANIMEVENTS
	if( scriptCommand->bodyPart[0] )
		Com_Printf( "anim0 (%s): %s\n", animBodyPartsStr[scriptCommand->bodyPart[0]].string, animModelInfo->animations[scriptCommand->animIndex[0]]->name );
	if( scriptCommand->bodyPart[1] )
		Com_Printf( "anim1 (%s): %s\n", animBodyPartsStr[scriptCommand->bodyPart[1]].string, animModelInfo->animations[scriptCommand->animIndex[1]]->name );
	Com_Printf("\n");
#endif
