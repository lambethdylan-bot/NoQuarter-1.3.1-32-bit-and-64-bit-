 ...
 * name:	g_client.c
 *
 * desc:	client functions that don't happen every frame
 *
 * NQQS:
 *
 */
...
================
core:
	SelectInfoPlayerSpawnPoint

	find a random info_player_start or info_player_intermission entity
================
*/
gentity_t *SelectInfoPlayerSpawnPoint( void ) {
	gentity_t	*spot = NULL;
	int			count = 0;
	gentity_t	*spots[MAX_SPAWN_POINTS];
	int			i;
	...
	for ( i=0; i<g_spawns.count; ++i ) {
		spot = g_spawns.ent[i];
		//if ( !spot->inuse ) continue;
		if ( spot->classnamehash != INFO_PLAYER_START_HASH && spot->classnamehash != INFO_PLAYER_INTERMISSION_HASH ) continue;
		//if ( Q_stricmp(spot->classname, "info_player_start") && Q_stricmp(spot->classname, "info_player_intermission") ) continue;
		spots[ count ] = spot;
		count++;
	}
...
gentity_t *SelectSpawnPoint( vec3_t avoidPoint, vec3_t origin, vec3_t angles ) {
	...
	if (!spot) {
		// core: if still nothing is found, try an info_player_start or info_player_intermission as a final option..
		spot = SelectInfoPlayerSpawnPoint();
		if ( !spot ) {
			G_Error( "Couldn't find a spawn point. Add a \"team_WOLF_objective\" and some \"team_CTF_redspawn\" / \"team_CTF_bluespawn\" entities." );
		}
	}
...
void G_DropLimboHealth(gentity_t *ent) {
...

	{
		...
		if(g_dropHealth.integer == -1) {
			packs = 5; // max packs
			ent->client->ps.classWeaponTime += (level.time - ent->client->deathTime + SECONDS_10);
		}
...
void limbo( gentity_t *ent, qboolean makeCorpse ) {
	if (!(ent->client->ps.pm_flags & PMF_LIMBO)) {
		...
		if ( makeCorpse ) {
			int contents = trap_PointContents( ent->r.currentOrigin, -1 ); // drop stuff
			if ( !(contents & CONTENTS_NODROP) ) {
				TossClientItems( ent );
			}
		}
	...
	switch ( original ) {
		...
		case WP_MP40:
			if ( HWSkill >= 4 && pc == PC_SOLDIER )
				...
				return team == TEAM_ALLIES ? WP_COLT : WP_LUGER;
		case WP_SHOTGUN:
			if ( g_soldierShotgun.integer && HWSkill >= 4 && pc == PC_SOLDIER )
				return WP_SHOTGUN;
			else if ( HWSkill >= 4 && pc == PC_SOLDIER )
				return team == TEAM_ALLIES ? WP_THOMPSON : WP_MP40;
			else if ( LWSkill >= 4 )
				return team == TEAM_ALLIES ? WP_AKIMBO_COLT : WP_AKIMBO_LUGER;
			else
...
void G_AddClassWeapons(gclient_t *client) {
	...
	if ( secondary == WP_NONE ) {
		...
		else if ( client->sess.skillBits[SK_LIGHT_WEAPONS]& (1<<4) && ( pc != PC_MEDIC || !(g_medics.integer & MEDIC_NOAKIMBO)) ) {
...
void SetWolfSpawnWeapons( gclient_t *client ) {
	...
	// core: except the footkick..
	memset(client->ps.ammo, 0, WP_FOOTKICK * sizeof(int));		// MAX_WEAPONS*sizeof(int)
	memset(client->ps.ammoclip, 0, WP_FOOTKICK * sizeof(int));	// "
...
void AddMedicTeamBonus( gclient_t *client ) {
	...
	// max HP depends on the # medics in a team..
		...
		if( client->pers.maxHealth > MAX_HEALTH ) {
			client->pers.maxHealth = MAX_HEALTH;
		}
	}

	if( client->sess.skillBits[SK_BATTLE_SENSE] & (1<<3) ) {
		client->pers.maxHealth += HEALTH_BONUS;
	}
...
char *CheckUserinfo( int clientNum, char *userinfo ) {
	...
	if(count == 0) {
		...
		// We check for empty names later (and replace them), so now don't return with an error message.
		//
...
char *CheckLocalhost( gentity_t *ent, char *userinfo ) {
	...
	if ( !(ent->r.svFlags & SVF_BOT) && nq_security.integer & NQS_ENABLE_LOCALHOST_HACK_CHECK) {
...
============
core:
	ClientConfigStringChanged()

	Send CS_PLAYERS ConfigString changes..
============
*/
char *ClientConfigStringChanged( gentity_t *ent ) {
	gclient_t	*client = ent->client;
	int			clientNum = client->ps.clientNum;
	char		*s;
	char		oldvalue[MAX_STRING_CHARS];
	char		skillStr[16] = "";
	char		medalStr[16] = "";
	char		skillBits[128] = "";
	int			i;

	// IRATA: no space for the last skillBit, saves us a char in each player CS
	for( i = 0; i < SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS; ++i ) { // all SK_NUM_SKILLS - SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS is last skill
	...
	Q_strcat( skillBits, sizeof(skillBits), va("%i", client->sess.skillBits[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] ) );
	Q_strcat( skillStr, sizeof(skillStr), va("%i",client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS]) );
	Q_strcat( medalStr, sizeof(medalStr), va("%i",client->sess.medals[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS]) );

	// send over a subset of the userinfo keys so other clients can
	// print scoreboards, display models, and play custom sounds
	s = va( "n\\%s\\t\\%i\\c\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%s\\dr\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\lc\\%i\\xp\\%s\\u\\%u",
		client->pers.netname,
	...
	trap_GetConfigstring( CS_PLAYERS + clientNum, oldvalue, sizeof(oldvalue) );

	if( !Q_stricmp(oldvalue, s) ) {
		// strings are equal, no need to send the configstring again..
		return NULL;
	}
...
// NOTE: why we have 2 reason vars ?
...
#ifdef HW_BAN
			case TOK_cg_hwguid:
				//only validate the hwguid when the user is not from the localhost
				if ( strcmp( client->pers.client_ip, "localhost" ) && nq_security.integer & NQS_USE_HW_BANS ) {
					if (strlen(cs_value) >= 32) {
						trap_Cvar_VariableStringBuffer("sv_hostname", serverName, sizeof(serverName));
						serverPort = trap_Cvar_VariableIntegerValue("net_port");

						hwGuid = getDecryptedHWGuid(cs_value, serverName, serverPort, clientNum);
						if (hwGuid == NULL) {
							G_Printf("HWGuid: decryption failed!\n");
							break;
						}

						for (i = 0; i < strlen(hwGuid); i++) {
							cs_value[i] = hwGuid[i];
						}

						cs_value[strlen(hwGuid)] = 0;

						G_Printf("HWGuid: client: %d IP: %s GUID: %s\n", clientNum, client->pers.client_ip, cs_value);

						if ( client->pers.cg_hwguid[0] && strcmp(client->pers.cg_hwguid, cs_value) != 0 ) {
							// They're trying to hack their guid...
							G_Printf("ClientUserinfoChanged: client %d hacking cg_hwguid, old=%s, new=%s\n", clientNum, client->pers.cg_hwguid, cs_value);
							trap_DropClient(clientNum, "Bad userinfo.", 0);
							break;
						}

						Q_strncpyz(cs_cg_hwguid, cs_value, MAX_STRING_CHARS);
					}
					else {
						G_Printf("ClientUserinfoChanged: Invalid HWGUID! client: %d IP: %s HWGUID: %s\n", clientNum, client->pers.client_ip, cs_value);
						trap_DropClient(clientNum, "Bad userinfo.", 0);
						break;
					}
				}
	...
	//only validate the hwguid when the user is not from the localhost
	if ( strcmp( client->pers.client_ip, "localhost" ) && nq_security.integer & NQS_USE_HW_BANS ) {
		//validate if the user is not HWGUID banned but only if he reach the connected state
		if (cs_cg_hwguid[0]) {
			if(G_shrubbot_hwban_check(cs_value, ban_reason)) {
				G_Printf("ClientUserinfoChanged: HWGuid banned! client: %d IP: %s HWGUID: %s\n", clientNum, client->pers.client_ip, cs_value);
				trap_DropClient(clientNum, va("You are banned from this server.\n%s", ban_reason), 0);
			}

			Q_strncpyz(client->pers.cg_hwguid, cs_cg_hwguid, PB_GUID_LENGTH+1);
		}
		else {
			if ( client->pers.connected == CON_CONNECTED ) {
				G_Printf("ClientUserinfoChanged: HWGuid missing! client: %d IP: %s\n", clientNum, client->pers.client_ip);
				trap_DropClient(clientNum, "Bad userinfo.", 0);
			}
			else {
				client->pers.cg_hwguid[0] = 0;
			}
		}
	}
#endif // HW_BAN
	...
	// censor name
	else if (g_censorNames.string[0] && !(ent->r.svFlags & SVF_BOT)) {
	...
	if ( !Info_Validate(userinfo) ) {
		...
		trap_DropClient(clientNum, "Invalid userinfo.", 300); // IRATA: 300 secs increase ?
	}
	...
	// core: adjust the CS_PLAYERS configstring..
	s = ClientConfigStringChanged(ent);
	if ( !s ) {
		return;
	}
	...
	// Log the UserInfos?
	if (g_logOptions.integer & LOGOPTS_USERINFO) {
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	if ( strlen(cs_name) == 0 ) {
		...
		Info_SetValueForKey( userinfo, "name", cs_name);
		trap_SetUserinfo( clientNum, userinfo );
		CP("cp \"You cannot assign an empty playername! (reset to default name)\"");
		G_LogPrintf( "ClientUserinfoChanged: %i User with empty name. (Changed to: \"Target #%i\")\n", clientNum, clientNum );
		G_DPrintf( "ClientUserinfoChanged: %i User with empty name. (Changed to: \"Target #%i\")\n", clientNum, clientNum );
	}
	...
	if (!isBot) {
		...
		// TODO: Test this. PB functions doesn't seem to be available on pb off servers ...
		if ( nq_security.integer & NQS_DISALLOW_CLIENTS_PB_OFF_CONNECT ) {
			if (strcmp(cs_guid, "unknown") == 0  || strcmp(cs_guid, "NO_GUID") == 0 ) {
					G_LogPrintf("nq security: User %s blocked with guid: %s \n", cs_name, cs_guid);
					return "Get a valid ET key at etkey.org or etkey.net and enable Punkbuster on your system.\nThis server requires player keys.";
			}
		}
		...
		if (nq_security.integer & NQS_DISABLE_DUPLICATE_GUID_KICK) { // FIXME: move this down and don't kick players with "NO_GUID"
			G_LogPrintf( "nq_security: Disconnect of duplicate guids deactivated.\n");
		}
		else {
...
//mcwf GeoIP
	...
	if ( firstTime ) {
		// do the connect announcer
		trap_SendServerCommand( -1, va("cpm \"%s" S_COLOR_WHITE " connected\n\"", client->pers.netname) );

		if (!isBot) {
			level.lastconnected_client = clientNum;
		}
	}

	// count current clients and rank for scoreboard
	CalculateRanks( qtrue, qfalse );
	client->ps.persistant[PERS_KILLS] = 0;

	// core: send the new weapons restriction data to the client..
	G_SendWeaponRestrictions( ent );
...
void ClientBegin( int clientNum ) {
	...
	int 		stat_xp, stat_xp_overflow; // IRATA: restore ps.stats
	char		*cs_guid	= NULL;
	qboolean    inIntermission = qfalse; // #592 IRATA: in intermission and ps.pm_type already == PM_INTERMISSION
	...
	// #592 IRATA: avoid double voting gametype 6 & moving client to intermission twice in case of intermission team change
	if (g_gamestate.integer == GS_INTERMISSION && client->ps.pm_type == PM_INTERMISSION) {
		inIntermission = qtrue;
	}

	// to restore xp value we did set in G_xpsave_load
	...
	// #592 IRATA: restore ps.pm_type - see comment above
	if (inIntermission == qtrue) {
		client->ps.pm_type = PM_INTERMISSION;
	}

	// restore the XP of ent->client->ps.stats[STAT_XP] (cg.xp) for the limbo menu at start
	// we might do it on team change as well ... and/or add more conditions?
	// TODO: check reconnect and xpsave off - test this!!!!
	if(ent->client->sess.spectatorState == SPECTATOR_FREE /*&& firsttime */ ) {
	...
	if ( client->sess.sessionTeam == TEAM_AXIS || client->sess.sessionTeam == TEAM_ALLIES ) {
		client->inactivityTime = level.time + ( g_inactivity.integer ? g_inactivity.integer : 60 ) * SECONDS_1;
		client->inactivitySecondsLeft = (g_inactivity.integer)? g_inactivity.integer : 60;
	}
	else {
		client->inactivityTime = level.time + ( g_spectatorInactivity.integer ? g_spectatorInactivity.integer : 60 ) * SECONDS_1;
	...
	// #592 IRATA: see DHM - Nerve - only during the match - don't reset client->ps.pm_type in intermission (PM_INTERMISSION)
	// this avoids the call of CG_Debriefing_Startup() twice in intermission if playere switches the team
	// fixes also ent->r.contents = CONTENTS_CORPSE in intermission (should be 0)
	// ent->health is pointless in intermission ...
	if(g_gamestate.integer != GS_INTERMISSION && client->sess.sessionTeam != TEAM_SPECTATOR &&
		level.time - level.startTime > SERVER_FRAMETIME_DELTA * GAME_INIT_FRAMES) {
...
void ClientSpawn( gentity_t *ent, qboolean revived, qboolean teamChange, qboolean restoreHealth) {
	...
	gclient_t			*client = ent->client;
	clientPersistant_t	savedPers; // restore the client state later on
	clientSession_t		savedSess; // restore the client state later on
	vec3_t				spawn_origin, spawn_angles; // revive
	int					i;
	// restore the client state later on
	int					savedPing;
	int					savedTeam;
	...
	int					flags;
	int					holdbreathtime = (client->sess.skillBits[SK_BATTLE_SENSE] & (1<<6)) ? HOLDBREATHTIME_LONG : HOLDBREATHTIME;
	qboolean			doWar = (nq_War.integer & WARMODE_ENABLE)? qtrue : qfalse;
	qboolean			inIntermission = qfalse; // #592 IRATA: in intermission and ps.pm_type already == PM_INTERMISSION
	...
	if( revived ) {
		...
		// credits - reset so they can buy again this life.
		client->sess.buyTime = 0; // IRATA: moved to !revive case
		...
		if( client->sess.sessionTeam != TEAM_AXIS && client->sess.sessionTeam != TEAM_ALLIES ) {
			...
			spawnPoint = SelectCTFSpawnPoint( client->sess.sessionTeam, client->pers.teamState.state, spawn_origin, spawn_angles, client->sess.spawnObjectiveIndex, !(nq_playerspawning.integer & PLAYERSPAWN_RANDOM) );
			// core: if there was no valid spawnpoint to be found, (free, enabled, visible & non-telefragging),
			// the player still needs a "spawnpoint" that will provide the camera-view (only).
			// cs: don't do this for bots. they don't know how to handle it.
			if (  !spawnPoint && !(ent->r.svFlags & SVF_BOT) ) {
				spawnPoint = SelectSpectatorSpawnPoint( spawn_origin, spawn_angles );
				if ( !spawnPoint ) {
	...
	for( i = 0 ; i < MAX_PERSISTANT ; ++i ) {
		savedPersistant[i] = client->ps.persistant[i];
	}

	// #592 IRATA: avoid double voting gametype 6 & moving clients to intermission twice in case of intermission team change
	if (g_gamestate.integer == GS_INTERMISSION && client->ps.pm_type == PM_INTERMISSION) {
		inIntermission = qtrue;
	}
	...
	for( i = 0 ; i < MAX_PERSISTANT ; ++i ) {
		client->ps.persistant[i] = savedPersistant[i];
	}
	//---
	...
	// IRATA #612: reset this so players don't burn after spawning
	ent->flameQuota = 0;
	...
	// TTimo: retrieve from the persistant storage (we use this in pmoveExt_t beause we need it in bg_*)
	...
	if( client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		if ( g_medics.integer & MEDIC_FASTRESUSCITATIONING && revived )
			client->ps.powerups[PW_INVULNERABLE] = level.time + SECONDS_1;
		else if(revived)
			client->ps.powerups[PW_INVULNERABLE] = level.time + SECONDS_3;
		else {
			// IRATA: team specific spawn Invul removed
			client->ps.powerups[PW_INVULNERABLE] = level.time + (g_spawnInvul.integer * SECONDS_1);
		}
	}
	// End Xian

	// IRATA: no need to update the character on revive
	// Note: For revive the client->ps.XXXAnim is not reset now - see G_UpdateCharacter()
	if (!revived) {
		G_UpdateCharacter( client );
	}
	...
	if(restoreHealth) {
		if( client->sess.skillBits[SK_BATTLE_SENSE] & (1<<3) )
			...
			ent->health = client->ps.stats[STAT_HEALTH] = client->ps.stats[STAT_MAX_HEALTH] - HEALTH_BONUS;
		else
	...
	if( !revived ) {
		...
		vec3_t	newangle = { 0, SHORT2ANGLE( ent->client->pers.cmd.angles[YAW] + ent->client->ps.delta_angles[YAW] ), 0};
	...
	client->inactivityTime = level.time + g_inactivity.integer * SECONDS_1;
	client->inactivityWarning = qfalse;
	client->inactivitySecondsLeft = (g_inactivity.integer)? g_inactivity.integer : 60;
	client->latched_buttons = 0;
	client->latched_wbuttons = 0;	//----(SA)	added

	client->deathTime = 0; // xkan, 1/13/2003 - reset death time
	...
	client->ps.commandTime = level.time - SERVER_FRAMETIME_DELTA;
	ent->client->pers.cmd.serverTime = level.time;

	// cs: spawn detection is handled client side now since this gets dropped most (all?) of the time
	// add spawn event
	...
	if ( teamChange && !(g_weapons.integer & WPF_LANDMINES_TIMEOUT) ) {
		G_ResetTeamMapData();
	}
}
...
void ClientDisconnect( int clientNum ) {
	...
	if( !(g_weapons.integer & WPF_LANDMINES_TIMEOUT) ) {
		G_FadeLandmines( ent );
	}
	G_FadeSatchels( ent );
	...
	// jaquboss/core
	// IRATA: free that shit in any case since we have different arrays now!
	// checks to do this for JP_INSANITY_MORTARCAM removed
	G_FadeMortars( ent );
	// IRATA: checks to do this for JP_INSANITY_MISSILECAM removed
	G_FadePanzerfausts( ent );
	G_FadeBazookas( ent );

	// IRATA: free/reset the player cam if there are any left (JP_INSANITY_KILLERCAM, JP_INSANITY_RIFLENADECAM)
	// just do it in any case - no sanity check required, see FreeCamera
	FreeCamera(clientNum);

	// remove ourself from teamlists
	G_RemoveTeamMapData( ent, qtrue, qtrue, qtrue );
	...
	if ( ent->client->pers.connected == CON_CONNECTED && ent->client->sess.sessionTeam != TEAM_SPECTATOR
		...
		if( item ) {
			vec3_t		launchvel = { 0, 0, 0 }; // OSP - fix for suicide drop exploit through walls/gates
	...
	// core: reset shortcut data..
	if ( level.lasthi_client == clientNum ) {
		level.lasthi_client = -1;
		level.lasthi_time = -1;
	}
	if ( level.lastbye_client == clientNum ) {
	...
	ent->client->ps.stats[STAT_XP] = 0; // IRATA
	ent->client->ps.stats[STAT_XP_OVERFLOW] = 0; // IRATA
	i = ent->client->sess.sessionTeam;
	ent->client->sess.sessionTeam = TEAM_FREE;
#ifdef SYSTEM_CHECK
	// IRATA: need to know if PB kicks finally call this function
	// nqClientSystemInfo init var is 0 see G_InitSessionData()
	// using -1 as indicator ...
	ent->client->sess.nqClientSystemInfo = -1;
#endif

#ifdef AUTO_GUID
	// IRATA: delete the nqkey on disconnect ... 
	Q_strncpyz(ent->client->sess.nqKeyInfo.nquid, "", PB_GUID_LENGTH+1);
#endif
