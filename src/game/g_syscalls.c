...
qboolean G_CvarUpdateSafe(const char *var_name, const char *value)
{
	const char *serverinfo_cvars[] = {
		...
		"g_gametype",
		"g_voteFlags",
//		"g_alliedmaxlives",			// functionality removed, but still used in ui
//		"g_axismaxlives",			// functionality removed, but still used in ui
		"g_minGameClients",
		"g_needpass",
		"sv_allowAnonymous",
		"sv_privateClients",
		...
		"protocol",
		"version",
//		"g_maxlivesRespawnPenalty", // functionality removed, but still used in ui
//		"g_maxGameClients",			// functionality removed
		"sv_maxclients",
		"timelimit",
//		"g_heavyWeaponRestriction", // functionality removed, but still used in ui
		"g_antilag",
//		"g_maxlives",				// functionality removed, but still used in ui
		"g_friendlyFire",
		"sv_floodProtect",
		"sv_maxPing",
		"sv_minPing",
...
IRATA:
MAX_GENTITIES_IN_A_BOX in a box has been added to use for 'maxcount' - max count of result. Within a small range (bounding boxes f.e.) we will never touch MAX_GENTITIES
because the range also limits the worldsectors which are touched recursively - see SV_AreaEntities_r
