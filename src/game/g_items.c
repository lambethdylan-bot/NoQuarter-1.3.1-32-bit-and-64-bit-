			  ...
			  int outOfReserve)	// is the amount to be added out of reserve
{
	...
	if (weapon < WP_LUGER || weapon >= WP_NUM_WEAPONS || weapon == WP_FOOTKICK)	// core: exclude WP_FOOTKICK..
		return qfalse;
...
// core: G_GetPrimaryWeaponForClient() is adjusted so it returns the
// correct primary weapon for high skilled soldiers..
weapon_t G_GetPrimaryWeaponForClient( gclient_t *client ) {
	...
	else if ( COM_BitCheck( client->ps.weapons, WP_JOHNSON ) )	{
		primary = WP_JOHNSON;
	}

	// core: fixing the soldier lvl 8 problem, to be able to pickup many weapons..
	// This needs the venom check first, followed by check for mp40/thompson
...
// core: G_GetSecondaryWeaponForClient() is adjusted so it returns the
// correct secondary weapon for high skilled soldiers..
weapon_t G_GetSecondaryWeaponForClient( gclient_t *client, weapon_t primary ) {
	...
	// core: a soldier lvl. 4+ can have an SMG or Shotgun as secondary weapon
	if ( secondary == WP_NONE && client->sess.playerType == PC_SOLDIER && client->sess.skillBits[SK_SOLDIER] & (1<<4)  ) {
		if ( primary != WP_NONE && primary != WP_THOMPSON && primary != WP_MP40 && primary != WP_SHOTGUN ) {
			if ( COM_BitCheck(client->ps.weapons, WP_THOMPSON) ) secondary = WP_THOMPSON;
			else if ( COM_BitCheck(client->ps.weapons, WP_MP40) ) secondary = WP_MP40;
			else if ( COM_BitCheck(client->ps.weapons, WP_SHOTGUN) ) secondary = WP_SHOTGUN;
		}
	}
...
weapon_t G_GetWeaponsForClient( gclient_t *client, weapon_t secondaryWeap ) {
	...
	// core: changed this function to use the other existing functions..
	// We want the same results, so use the same checks.
	primary = G_GetPrimaryWeaponForClient( client );
	secondary = G_GetSecondaryWeaponForClient( client, primary );
#ifdef DEBUG
...
// basically changed *LauchItem, maybe cleaner if we use this ...
gentity_t *LaunchDogTag( gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum ) {
	trace_t		tr;
	vec3_t		vec, temp;
	...
	dropped->nextthink = level.time + SECONDS_15;
	dropped->flags = FL_DROPPED_ITEM;
	trap_LinkEntity (dropped);
	return dropped;
}

// basically changed G_DropWeapon, maybe cleaner if we use this
void G_DropDogTag( gentity_t *ent ) {
	vec3_t		angles, velocity, org, offset, mins, maxs;
	gclient_t	*client = ent->client;
	gentity_t	*ent2;
	gitem_t		*item = &bg_itemlist[IL_WEAPON_DOGTAGS];	// no need to search for the item, using BG_FindItemForWeapon(ITEM_DOGTAGS)..
	trace_t		tr;
	...
	ent2 = LaunchDogTag( item, org, velocity, client->ps.clientNum );
}
...
qboolean G_CanPickupWeapon( weapon_t weapon, gentity_t* ent ) {
	...
	// possible to pickup dog tags?..
	// Dog tags are not real weapons, but items of type IT_WEAPON.
	if ( weapon == ITEM_DOGTAGS ) {
		// dog tags enabled?
		return (jp_insanity.integer & JP_INSANITY_DOGTAGS)? qtrue : qfalse;
	}
	...
	switch (ent->client->sess.playerType) {
		case PC_SOLDIER:
			if ( g_unlockWeapons.integer & UNLOCK_WEAPONS_SOLDIER ) return qtrue;
		...
		case PC_MEDIC:
			if ( g_unlockWeapons.integer & UNLOCK_WEAPONS_MEDIC ) return qtrue;
		...
		case PC_ENGINEER:
			if ( g_unlockWeapons.integer & UNLOCK_WEAPONS_ENGINEER ) return qtrue;
			if ( IS_AKIMBO_PISTOL(weapon) && ent->client->sess.skillBits[SK_LIGHT_WEAPONS] & (1<<4) ) return qtrue;
			if ( IS_PISTOL(weapon) ) return qtrue;
			return IS_ENGINEER_WEAPON(weapon);

		case PC_FIELDOPS:
			if ( g_unlockWeapons.integer & UNLOCK_WEAPONS_FIELDOPS ) return qtrue;
			if ( IS_AKIMBO_PISTOL(weapon) && ent->client->sess.skillBits[SK_LIGHT_WEAPONS] & (1<<4) ) return qtrue;
			if ( IS_PISTOL(weapon) ) return qtrue;
			return IS_FIELDOPS_WEAPON(weapon);

		case PC_COVERTOPS:
			if ( g_unlockWeapons.integer & UNLOCK_WEAPONS_COVERTOPS ) return qtrue;
...
int Pickup_Weapon( gentity_t *ent, gentity_t *other ) {
	...
	if( ent->item->giTag == WP_AMMO ) {
		...
		// core: don't allow picking up ammo if NQ warmode is on..
	...
	// picking up dog tags..
	if ( ent->item->giTag == ITEM_DOGTAGS ) {
		// add 1 to the counter..
		other->client->ps.stats[STAT_DOGTAGS]++;
		other->client->sess.dogtags++;
		return RESPAWN_NEVER;
	}
	...
	if( alreadyHave ) {
		...
		if( level.time - other->client->dropWeaponTime < SECONDS_1 )
			return 0;

		if( IS_SET_WEAPON(other->client->ps.weapon) )
			return 0;


		// See if we can pick it up
		if( G_CanPickupWeapon( ent->item->giTag, other )) {
			qboolean soldier4			= (other->client->sess.playerType == PC_SOLDIER && other->client->sess.skillBits[SK_SOLDIER] & (1<<4));
			qboolean soldier8			= (other->client->sess.playerType == PC_SOLDIER && other->client->sess.skillBits[SK_SOLDIER] & (1<<8));
			weapon_t primaryWeapon		= G_GetPrimaryWeaponForClient( other->client );
			weapon_t secondary			= G_GetSecondaryWeaponForClient( other->client, primaryWeapon );
			qboolean hasPrimary			= (primaryWeapon != WP_NONE);
			qboolean hasSecondary		= (secondary != WP_NONE);
			qboolean hasPrimarySMG		= IS_SUBMACHINEGUN(primaryWeapon);
			qboolean hasSecondarySMG	= IS_SUBMACHINEGUN(secondary);
			qboolean hasAkimbo			= IS_AKIMBO_PISTOL(secondary);
			qboolean hasPistol			= IS_PISTOL(secondary);
			qboolean pickupSMG			= IS_SUBMACHINEGUN(ent->item->giTag);
			qboolean pickupPistol		= IS_SECONDARY(ent->item->giTag);
			// qboolean pickupAkimbo		= IS_AKIMBO_PISTOL(ent->item->giTag);
			qboolean wantSMG			= (IS_SUBMACHINEGUN(other->client->sess.latchPlayerWeapon) || IS_SUBMACHINEGUN(other->client->sess.latchPlayerWeapon2));
			qboolean wantShotgun		= (other->client->sess.latchPlayerWeapon == WP_SHOTGUN || other->client->sess.latchPlayerWeapon2 == WP_SHOTGUN);
			qboolean wantAkimbo			= IS_AKIMBO_PISTOL(other->client->sess.latchPlayerWeapon2);
			qboolean wantPistol			= IS_PISTOL(other->client->sess.latchPlayerWeapon2);

			// soldiers level 4+ can have secondary SMG.
			// soldiers level 8+ can pickup any weapon, and -can- have an SMG + akimbo.
			// Check what weapons they prefer (selected in limbo), and
			// try to keep those weapons (if possible).
			if ( soldier4 ) {
				// picking up pistol(s)..
				if ( pickupPistol ) {
					if ( hasSecondary ) G_DropWeapon( other, secondary );
				}
				// picking up SMG..
				else if ( pickupSMG ) {
					if ( soldier8 || (g_unlockWeapons.integer & UNLOCK_WEAPONS_SOLDIER) ) {
						if ( hasPrimarySMG ) G_DropWeapon( other, primaryWeapon );
						else if ( hasSecondarySMG ) G_DropWeapon( other, secondary );
						else if ( wantAkimbo && hasPrimary && hasAkimbo ) G_DropWeapon( other, primaryWeapon );
						else if ( wantAkimbo && hasPrimary && hasSecondary && !hasAkimbo ) G_DropWeapon( other, secondary );
						else if ( (wantSMG || wantShotgun) && hasPrimary && hasAkimbo ) G_DropWeapon( other, secondary );
						else if ( (wantSMG || wantShotgun) && hasPrimary && hasSecondary && !hasAkimbo ) G_DropWeapon( other, secondary );
						else if ( wantPistol && hasPrimary && hasPistol ) G_DropWeapon( other, primaryWeapon );
						else if ( wantPistol && hasPrimary && hasSecondary && !hasPistol ) G_DropWeapon( other, secondary );
						else if ( hasPrimary && hasSecondary ) G_DropWeapon( other, secondary );
					} else {
						if ( hasPrimarySMG ) G_DropWeapon( other, primaryWeapon );
						else if ( hasSecondarySMG ) G_DropWeapon( other, secondary );
						else if ( hasSecondary ) G_DropWeapon( other, secondary );
					}
				}
				// picking up some primary weapon..
				else {
					if ( (wantSMG || wantShotgun) && (hasPrimarySMG || primaryWeapon == WP_SHOTGUN) && hasSecondary ) G_DropWeapon( other, secondary );			// keep the SMG/Shotgun, it will become a secondary weapon
					else if ( (wantSMG || wantShotgun) && (hasSecondarySMG || secondary == WP_SHOTGUN) && hasPrimary ) G_DropWeapon( other, primaryWeapon );	// switch the primary weapon
					else if ( wantAkimbo && hasPrimarySMG && hasAkimbo ) G_DropWeapon( other, primaryWeapon );	// drop the SMG, keep akimbo
					else if ( hasPrimary ) G_DropWeapon( other, primaryWeapon );
				}
			} else {
				// no soldier lvl. 4
				if ( hasPrimary && !pickupPistol)			// dont drop primary if we pickup secondary
					G_DropWeapon( other, primaryWeapon );	// drop our primary weapon
				else if ( hasSecondary && pickupPistol)
			...
			switch (ent->item->giTag) {
				...
				case WP_K43:
					...
					break;
				case WP_JOHNSON:
					COM_BitSet( other->client->ps.weapons, WP_JOHNSON_SCOPE );
					break;
...
int Pickup_Powerup(gentity_t *ent, gentity_t *other) {
	...
	if ( ent->item->giTag == PW_OPS_DISGUISED ) {
		...
		Q_strncpyz(other->client->disguiseNetname, other->client->pers.netname, sizeof(other->client->disguiseNetname));

		other->client->disguiseRank = other->client->sess.rank;

		// jaquboss - these needs to be forwarded
		ClientConfigStringChanged( other );
...
int Pickup_Health(gentity_t *ent, gentity_t *other) {
	int			max = other->client->ps.stats[STAT_MAX_HEALTH];
	int			quantity = 0;
		...
		if( ent->parent && ent->parent->client && other->client->sess.sessionTeam == ent->parent->client->sess.sessionTeam ) {
			...
			if ( g_medics.integer & MEDIC_MIRRORHEAL ) {
				...
				if ( ent->parent->health > 0 &&
					...
					(other->client->lasthurt_client == ent->parent->s.number && level.time > other->client->lasthurt_time + SECONDS_30)) )
				{
...
//	G_LogPrintf( "Finished pickup function\n" );
	...
	if ( ent->item->giType == IT_TEAM ) {
		gentity_t* te = G_TempEntityNotLinked( EV_GLOBAL_ITEM_PICKUP );
...
//	G_LogPrintf( "Firing item targets\n" );
	...
	if ( respawn <= 0 ) {
		...
		ent->nextthink = level.time + respawn * SECONDS_1;
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum, int flags ) {
	...
	if (item->giType == IT_TEAM) {
		...
		dropped->nextthink = level.time + SECONDS_30;
...
void G_SpawnItem (gentity_t *ent, gitem_t *item) {
	...
	ent->nextthink = level.time + SERVER_FRAMETIME_DELTA * 2;
...
void G_FlushItem( gentity_t *ent, trace_t *trace ) {
	if ( g_flushItems.integer && ent->physicsFlush && trace->plane.normal[2] > 0.7f &&
...
void G_BounceItem( gentity_t *ent, trace_t *trace ) {
	...
	if ( trace->plane.normal[2] >= 0.7 || VectorLength( ent->s.pos.trDelta ) < 16 || !ent->physicsSlide ) {
		...
		// FIXME WP_JOHNSON
		else if ( ent->item && ( ent->item->giType == IT_WEAPON || ent->item->giType == IT_AMMO ) && ent->item->giTag > WP_NONE && ent->item->giTag < WP_NUM_WEAPONS && ent->item->giTag != WP_FOOTKICK) {
...
qboolean G_CheckForCabinetResupply( gentity_t *ent, gentity_t *traceEnt ) {
	...
	for ( i=0; i<g_cabinets.count; i++ ) {
		tent = g_cabinets.ent[i];
		if ( !tent->target_ent ) continue;
		if ( tent->target_ent->s.number != traceEnt->s.number ) continue;
		if ( tent->count == -9999 ) return qfalse;	// unlimited cabinet?
		if ( tent->health >= tent->count ) return qfalse;	// can the cabinet be filled any more?

		// add it if this is not unlimited cabinet
		if ( type == TYPE_HEALTH ) {
			tent->health += ent->item->quantity;
			if ( tent->health > tent->count ) {
				tent->health = tent->count;
			}
		}
		else {
			tent->health += ent->count;
			if ( tent->health > tent->count ) {
				tent->health = tent->count;
			}
		}

		// remove the health/ammo-pack
		G_FreeEntity( ent );
		return qtrue;
	}
	// next line should never be executed.. just being formal.
	return qfalse;
}
...
void G_RunItem( gentity_t *ent ) {
	...
	trap_LinkEntity( ent );	// FIXME: avoid this for stationary?
							// core: a check has already been done: if ( ent->s.pos.trType == TR_STATIONARY ) ... return;
