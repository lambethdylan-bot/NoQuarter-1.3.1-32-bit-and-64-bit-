 ...
 * name:	g_crash.c
 *
 * desc:	game crash code
 *
 *
 * NQQS:	Fix x86_64 warnings
 */
...
void linux_siginfo(int signal, siginfo_t *siginfo) {
	G_CrashLogPrintf("Signal: %s (%d)\n", strsignal(signal), signal);
	G_CrashLogPrintf("Siginfo: %p\n", siginfo);
	if(siginfo) {
		G_CrashLogPrintf("Code: %d\n", siginfo->si_code);
		G_CrashLogPrintf("Faulting Memory Ref/Instruction: %p\n",siginfo->si_addr);
	}
}
...
void linux_dsoinfo() {
	...
	G_CrashLogPrintf("DSO Information:\n");
...
void linux_backtrace(ucontext_t *ctx) {
	...
	#ifdef GLIBC_21
		G_CrashLogPrintf("Stack frames: %Zd entries\n", size-1);
		array[1] = (void *)ctx->uc_mcontext.gregs[EIP];
	#else
		G_CrashLogPrintf("Stack frames: %zd entries\n", size-1);

		#if defined(__x86_64__) // might break -m32 builds on 64bit
			// IRATA: x86_64 defines REG_RIP instead.
			#ifdef REG_RIP
			array[1] = (void *)ctx->uc_mcontext.gregs[REG_RIP];
			#else
			array[1] = (void *)ctx->uc_mcontext.gregs[16];
			#endif
		#else
			#ifdef REG_EIP
			array[1] = (void *)ctx->uc_mcontext.gregs[REG_EIP];
			#else
			array[1] = (void *)ctx->uc_mcontext.gregs[14];
			#endif
		#endif

	#endif// GLIBC_21
	G_CrashLogPrintf("Backtrace:\n");
...
__sighandler_t INTHandler(int signal, struct sigcontext ctx) {
	G_CrashLogPrintf("------------------------------------------------\n");
	G_CrashLogPrintf("Ctrl-C is not the proper way to kill the server.\n");
	G_CrashLogPrintf("------------------------------------------------\n");
	return 0;
}
...
void CrashHandler(int signal, siginfo_t *siginfo, ucontext_t *ctx) {
	...
	if(segvloop < 2) {
		...
		linux_dsoinfo();
		linux_backtrace(ctx);
		G_CrashLogPrintf("-8<--------------------------------->8-\n\n");
		G_CrashLogPrintf("Attempting to clean up.\n");
		G_ShutdownGame(0);
...
void win32_exceptioninfo(LPEXCEPTION_POINTERS e) {
	G_CrashLogPrintf("Exception: %s (0x%08lx)\n", ExceptionName(e->ExceptionRecord->ExceptionCode), e->ExceptionRecord->ExceptionCode);
	G_CrashLogPrintf("Exception Address: %p\n", e->ExceptionRecord->ExceptionAddress);
}

void win32_dllinfo() {
	G_CrashLogPrintf("DLL Information:\n");
...
void win32_backtrace(LPEXCEPTION_POINTERS e) {
	...
	while(1) {
		...
		if(dwModBase) {
			GetModuleFileName((HINSTANCE)dwModBase, modname, MAX_OSPATH);
		}
		else {
...
void win32_initialize_handler(void) {
	...
	if(!imagehlp) {
		G_CrashLogPrintf("imagehlp.dll unavailable\n");
		return;
	}
	...
	if(
		...
		G_CrashLogPrintf("imagehlp.dll missing exports.\n");
		return;
	}
