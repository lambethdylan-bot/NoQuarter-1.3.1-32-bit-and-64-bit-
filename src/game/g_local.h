...
// see also MAX_SCRIPT_ACCUM_BUFFERS
...
//unlagged - true ping
#define NUM_PING_SAMPLES 64	// core: was 64, then 32, now 64 again..
							// note: keep this value a power-of-2. see ClientThink_real() for details..
//unlagged - true ping
...
// NQKey
typedef struct {
	char				nquid[PB_GUID_LENGTH+1];	// IRATA
	// char				client_ip[MAX_IP_LENGTH] // storing IP to verify
	// int timeSet;
} nqKeyInfo_t;
#endif
...
entityarray_t g_cabinets;		// all health- & ammo-cabinets..
//---
...
typedef struct playerTeamState_s
{
	...
	int						location[3];
/* IRATA: The following vars are obviously never used and/or only set in 'game'
   WIP or relict? I'll keep the code for now ... do a code search for var names
	int						captures; // no use
	int						basedefense; // only set, not used
	int						carrierdefense; // unused!
	int						flagrecovery; // only set, not used
	int						fragcarrier; // only set, not used
	int						assists; // not used

	float					lasthurtcarrier; // remove! only set, not used!
	float					lastreturnedflag; // remove! only set, not used!
	float					flagsince; // remove! only set, not used!
	float					lastfraggedcarrier;  // remove! only set, not used!
*/
} playerTeamState_t;
...
// IRATA:
#define NOQ_DONT_WRITE			1		// don't write to shrubbot
#define NOQ_DONT_READ_ADMINS	2		// don't read admins
#define NOQ_DONT_READ_BANS		4		// don't read bans
#define NOQ_DONT_READ_MUTES		8		// don't read mutes
...
typedef struct {
	...
	int					skillBits[SK_NUM_SKILLS];		// jaquboss available skills in bits
	unsigned int 		uci; 				//mcwf GeoIP

	// credits - no transmitting
	int					credits;
	gentity_t			*currentBetTarget;
	int					currentBetAmount;
	int					betTime;
	int					buyTime;		// cleared on spawn. one buy per life

#ifdef SYSTEM_CHECK
	int					nqClientSystemInfo; // no transmitting
#endif

#ifdef AUTO_GUID
	nqKeyInfo_t 		nqKeyInfo;	// NQkey per slot
#endif
...
//unlagged - true ping
	...
	int					samplehead;
	int					pingsamplessum;	// core: added to eliminate the constant pingsamples[] looping
//unlagged - true ping
...
struct gclient_s
{
	...
	// int			lasthurt_location;	// jet Pilot - location (head, torso, leg, etc.) of last damage // IRATA: this is set but never used see g_combat.c
	int			lastrevive_client;
	int			lastkiller_client;
	int			lastammo_client;
	int			lasthealth_client;
...
// see also G_MAX_SCRIPT_ACCUM_BUFFERS
...
//jet Pilot - moved level_locals_t declaration to g_local.h
typedef struct level_locals_s
{
	struct gclient_s	*clients;					// [maxclients]
	struct gentity_s	*gentities;
	//	int				gentitySize; 				// IRATA: unused
	int					num_entities;				// current number, <= MAX_GENTITIES
	int					warmupTime;					// restart match at this time
	fileHandle_t		chatLog;					// jet Pilot - separate logfile specific to chats
	// fileHandle_t		tkLog; 						// IRATA: unused
	...
	int					warmupModificationCount;	// for detecting if g_warmup is changed // IRATA: not used for real ...
	...
	int					intermissionQueued;		// intermission was qualified, but
												// wait INTERMISSION_DELAY_TIME = 1000 (1 second) before
												// actually going there so the last
												// frag can be watched.  Disable future
												// kills during this delay
	int					intermissiontime;		// time the intermission was started
	char				*changemap;				// IRATA: not used for real
	int					exitTime;
...
// ===================
	...
	gentity_t			*tempTraceIgnoreEnts[ MAX_GENTITIES ];
	int					numTempTraceIgnoreEnts;
	...
	// core: shortcuts for last client saying bye/hi
	int					lastconnected_client;
	int					lastbye_client;
	int					lastbye_time;
	int					lasthi_client;
	int					lasthi_time;
...
qboolean G_FollowSame( gentity_t *ent ); //core: added..
...
// void SetRespawn(gentity_t *ent, float delay); // IRATA: not implemented
...
// qboolean G_CheckForExistingModelInfo( bg_playerclass_t* classInfo, const char *modelName, animModelInfo_t **modelInfo ); // IRATA: not implemented
...
// void Cmd_Insanity_Add(gentity_t *ent); // IRATA: not implemented
...
// qboolean ClientOutOfAmmo( int client ); // IRATA: not implemented
...
// to call airstrike? IRATA: not implemented
// qboolean ReadyToCallAirstrike(gentity_t* ent);
// to use smoke grenade? IRATA: not implemented
...
// extern vmCvar_t		g_landminetimeout; // IRATA: see g_weapons 32
...
// extern vmCvar_t 	g_forceLimboHealth; // IRATA: removed
...
#ifdef NQ_TEST
extern vmCvar_t nq_test;	// a temporary test cvar. (the following notes are not up to date!)
							// bit 0 = antilag time - own ping
							// bit 1 = antilag time - other ping
							// bit 2 = antilag time - 50 ms (build-in lag)
							// bit 3 = final spread / 2
#endif

extern vmCvar_t nq_noq; // disable shrubbot user data read/write

extern vmCvar_t nq_security; // nq_security
...
// int	trap_GeneticParentsAndChildSelection(int numranks, float *ranks, int *parent1, int *parent2, int *child); // IRATA: not implemented
...
// qboolean G_allowPanzer(gentity_t *ent); // IRATA not implemented
...
int G_WeaponCount( gentity_t* ent, weapon_t weap );	// core: was called G_TeamCount before..
...
#define SIF_OPTIMIZED				32	// Optimized - Don't display jp_insanity, g_skills


// tjw: g_goombaFlags
#define GBF_ENEMY_ONLY				1	// goomba does only damage enemies
#define GBF_NO_HOP_ENEMYDAMAGE		2	// EV_FALL_SHORT does not do damage to enemies
#define GBF_NO_HOP_TEAMDAMAGE		4	// EV_FALL_SHORT does not do damage to teammates
...
#define LOGOPTS_USERINFO		2		// Log client userinfo changes
#define LOGOPTS_BOTS			4		// Log bot initiated actions
#define LOGOPTS_CHATS			8		// Log chats
#define LOGOPTS_PICKUPS			16		// Log item pickups
...
#define LOGOPTS_LEVELTIME		2048	// Log the level time (see also LOGOPTS_REALTIME)
										// default is  level.time,
										// this flag prints level.time-level.startTime
...
// g_script_actions.c
extern int redlimbotime;
extern int bluelimbotime;
...
// core: Create a text+voice announce for killingsprees/multikill/firstblood/firstheadshot
void C_TextVoiceAnnounce( unsigned char flags, unsigned char data, int sound, int playerNum );

// IRATA: bitflags for nq_security cvar
#define NQS_DISALLOW_CLIENTS_PB_OFF_CONNECT 0x0001
#ifdef HW_BAN
  #define NQS_USE_HW_BANS 0x0002
#endif // HW_BAN
#define NQS_ENABLE_LOCALHOST_HACK_CHECK 0x0004
// temp fix for xreal guids ... there are none atm :/
#define NQS_DISABLE_DUPLICATE_GUID_KICK 0x0008

// core: g_client.c
// Updates the CS_PLAYERS configstring for a player..
char *ClientConfigStringChanged( gentity_t *ent );

// core: g_main.c
// Send new weapons restriction data to a connecting player..
void G_SendWeaponRestrictions( gentity_t *ent );
// set the restrictions for one weapon..
...
// write the weapon restrictions file..
...
// core: g_main.c
// clear/set a bit in a bitflagged cvar..
