...
void G_Script_ScriptParse( gentity_t *ent )
{
	...
	qboolean				inScript;
	int						eventNum;
	static g_script_event_t	events[G_MAX_SCRIPT_STACK_ITEMS];
	int						numEventItems;
...
void G_Script_ScriptEvent( gentity_t *ent, gScriptEvent_t eventStr, char *params )
{
	...
	switch (eventStr) {
		case SE_TRIGGER:
			...
			if (g_logOptions.integer & LOGOPTS_SCRIPTEVENTS) {
...
void script_mover_think( gentity_t *ent )
{
	if( ent->spawnflags & MOVER_MOUNTEDGUN ) {
		if( !ent->tankLink ) {
			if( ent->mg42weapHeat ) {
				ent->mg42weapHeat -= (300.f * SERVER_FRAMETIME_DELTA * 0.001);

				if( ent->mg42weapHeat < 0 )
					ent->mg42weapHeat = 0;
			}
			if( ent->backupWeaponTime ) {
				ent->backupWeaponTime -= SERVER_FRAMETIME_DELTA;
...
void script_mover_use(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	if(ent->spawnflags & MOVER_RESURECTABLE) {
		if(ent->count) {
			...
			// core: like G_Damage(): targ->s.dl_intensity = 255.f * ((float)(targ->health) / (float)(targ->count));
			// this dl_intensity is actually sent to clients as a byte..
...
void SP_script_multiplayer(gentity_t *ent)
{
	...
	level.gameManager->s.otherEntityNum = teammaxLandmines;		// axis landmine count
	level.gameManager->s.otherEntityNum2 = teammaxLandmines;	// allies landmine count
