...
void CG_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor )
{
	...
	if ( icon ) {
		...
		// Adds Weapon Icon Color Change In Diff States - Hawkeye
		
		// 1. Grab the exact weapon state from the player
		int ws = cg.predictedPlayerState.weaponstate;

		// 2. Override the incoming 'hcolor' based on what the gun is doing
		if (ws == WEAPON_FIRING || ws == WEAPON_FIRINGALT) {
			VectorCopy(colorRed, hcolor);
			hcolor[3] = 1.0f; // Ensure full opacity
		}
		else if (ws == WEAPON_READY) {
			VectorCopy(colorYellow, hcolor);
			hcolor[3] = 1.0f;
		}
		else if (ws == WEAPON_RELOADING || ws == WEAPON_DROPPING || ws == WEAPON_RAISING) {
			VectorCopy(colorMdGrey, hcolor);
			hcolor[3] = 1.0f;
		}
		else {
			VectorCopy(colorWhite, hcolor); // Default to white while switching, etc.
			hcolor[3] = 1.0f;
		}

		// 3. Paint the image using our new forced colors
		trap_R_SetColor(hcolor);
		CG_DrawPic(x, y, w, h, icon);

		// 4. Clear the color so it doesn't bleed onto the rest of your HUD!
