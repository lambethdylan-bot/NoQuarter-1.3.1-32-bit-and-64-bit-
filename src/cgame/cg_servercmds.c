...
void InitSkillLevelStructure( skillType_t skillType ) {
	...
	// make sure we have 9 positive integers in ascending order, or its not valid
	// core: ignore -1 values, those are markers for a disabled skill..
	if( !(count == 9 && levels[0] >= -1 &&
			(levels[1] == -1 || levels[1] >= levels[0]) &&
			(levels[2] == -1 || levels[2] >= levels[1]) &&
			(levels[3] == -1 || levels[3] >= levels[2]) &&
			(levels[4] == -1 || levels[4] >= levels[3]) &&
			(levels[5] == -1 || levels[5] >= levels[4]) &&
			(levels[6] == -1 || levels[6] >= levels[5]) &&
			(levels[7] == -1 || levels[7] >= levels[6]) &&
			(levels[8] == -1 || levels[8] >= levels[7]) ) )
	{
		cg.xpOrder = qtrue;
	}
...
void CG_UpdateForceCvars( void ) {
	...
	if (cg.forceCvarCount > MAX_FORCECVARS) {
		cg.forceCvarCount = MAX_FORCECVARS;
	}
	...
	for ( i=0; i<cg.forceCvarCount; i++){
		...
		if (cg.cvarBackupsCount < (MAX_FORCECVARS+MAX_SVCVARS)) {
...
void CG_UpdateSvCvars( void ) {
	...
	if (cg.svCvarCount > MAX_SVCVARS) {
		cg.svCvarCount = MAX_SVCVARS;
	}
...
void C_ResetWeaponRestrictions( void ) {
	...
	for (i=WP_NONE; i<WP_NUM_WEAPONS; ++i) {
		weapon_restrictions[i].disabled = qfalse;
		weapon_restrictions[i].restricted = qfalse;
		weapon_restrictions[i].r_minplayers = qfalse;
		weapon_restrictions[i].r_maxweap = qfalse;
		weapon_restrictions[i].r_percentage = qfalse;
...
==================
core:
CG_WeaponRestrictions

received weapon restriction data from the server..
==================
*/
void C_WeaponRestrictions( void ) {
	int	argc = trap_Argc();
	int	i;
	...
	// overwrite with the received data..
	for (i=1; i<argc; i+=4) {
		...
		if (value <= 1) {
			weapon_restrictions[WeaponIndex].r_minplayers = qfalse;
		} else {
			if (value >= cgs.maxclients) {
				// more players needed than can join the server before weapon is activated.. in other words: it's disabled
				weapon_restrictions[WeaponIndex].disabled = qtrue;
				weapon_restrictions[WeaponIndex].r_minplayers = qtrue;
			} else {
				weapon_restrictions[WeaponIndex].minplayers = value;
				weapon_restrictions[WeaponIndex].r_minplayers = qtrue;
			}
		}
		...
		value = atoi( CG_Argv(i+2) );
		if (value < 0) {
			weapon_restrictions[WeaponIndex].r_maxweap = qfalse;
		} else {
			if (value == 0) {
				// max. 0 of this weapon ingame.. in other words: it's disabled
				weapon_restrictions[WeaponIndex].disabled = qtrue;
				weapon_restrictions[WeaponIndex].r_maxweap = qtrue;
			} else if (value < cgs.maxclients) {
				weapon_restrictions[WeaponIndex].maxweap = value;
				weapon_restrictions[WeaponIndex].r_maxweap = qtrue;
			}
		...
		if (value < 0 || value >= 100) {
			weapon_restrictions[WeaponIndex].r_percentage = qfalse;
		} else {
			if (value == 0) {
				// 0 percent is allowed to use the weapon.. in other words: it's disabled
				weapon_restrictions[WeaponIndex].disabled = qtrue;
				weapon_restrictions[WeaponIndex].r_percentage = qtrue;
			} else {
				weapon_restrictions[WeaponIndex].percentage = value;
				weapon_restrictions[WeaponIndex].r_percentage = qtrue;
			}
		}
	}
...
static void CG_ParseWarmup( void ) {
	...
	if(warmup == 0 && cg.warmup) {
		...
		// cs: moved to CG_DrawWarmup (bleh) so it is not g_doWarmup dependant 
...
void CG_ParseOIDInfo( int num ) {
	...
	// Bounds check: index must be within the oidInfo array
	if ( index < 0 || index >= MAX_OID_TRIGGERS ) {
		return;
	}
	...
	cgs.oidInfo[ index ].customimageallies = 0;	// set default handle to 0, in case no "a" param is found..
	cs = Info_ValueForKey( info, "a" ); // "cia"
	if( cs && *cs ) {
		int shaderIdx = atoi( cs );
		if ( shaderIdx >= 0 && shaderIdx < MAX_CS_SHADERS )
			cgs.oidInfo[ index ].customimageallies = cgs.gameShaders[shaderIdx];
	}

	cgs.oidInfo[ index ].customimageaxis = 0;	// set default handle to 0, in case no "b" param is found..
	cs = Info_ValueForKey( info, "b" ); // "cix"
	if( cs && *cs ) {
		int shaderIdx = atoi( cs );
		if ( shaderIdx >= 0 && shaderIdx < MAX_CS_SHADERS )
			cgs.oidInfo[ index ].customimageaxis = cgs.gameShaders[shaderIdx];
	}
...
void CG_ParseSpawns( void ) {
	...
	// Bounds check: custom maps may report more spawn targets than MAX_SPAWNPOINTS
	if ( cg.spawnCount > MAX_SPAWNPOINTS ) {
		CG_Printf( "^3WARNING: CG_ParseSpawns: map has %i spawn targets, clamping to MAX_SPAWNPOINTS (%i)\n", cg.spawnCount, MAX_SPAWNPOINTS );
		cg.spawnCount = MAX_SPAWNPOINTS;
	}
...
static void CG_AddToTeamChat( const char *str, int clientnum, qboolean disguise ) {
	...
	// core: make the chat text fit over as much available width as possible,
	// so texts are less often wrapped over multiple lines..
	...
	if (cg_teamChatHeight.integer < TEAMCHAT_HEIGHT) {
		// force TEAMCHAT_HEIGHT in intermission
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats ) {
	...
	// IRATA: engine doesn't support sich feature
...
void CG_LoadVoiceChats( void ) {
	int numTotalAllies = 0;
	int numTotalSoundsAxis = 0;
	int i;
	...
	// IRATA: show number of sounds and sound chat lists
	for (i = 0; i < voiceChatLists[0].numVoiceChats; ++i) {
		numTotalSoundsAxis += voiceChatLists[0].voiceChats[i].numSounds;
	}
	CG_Printf("... %i axis voice chat entries loaded - total sounds: %i (max %i)\n", voiceChatLists[0].numVoiceChats, numTotalSoundsAxis,MAX_VOICECHATS );

	for (i = 0; i < voiceChatLists[0].numVoiceChats; ++i) {
		numTotalAllies += voiceChatLists[1].voiceChats[i].numSounds;
	}
	CG_Printf("... %i allies voice chat entries loaded - total sounds:  %i (max %i)\n", voiceChatLists[1].numVoiceChats , numTotalAllies,MAX_VOICECHATS);

	CG_Printf("... %i total voice chat entries with %i sounds loaded.\n", (voiceChatLists[0].numVoiceChats + voiceChatLists[1].numVoiceChats), (numTotalSoundsAxis + numTotalAllies));
}
...
void CG_PlayBufferedVoiceChats( void ) {
	if ( cg.voiceChatTime < cg.time ) {
		if (cg.voiceChatBufferOut != cg.voiceChatBufferIn && voiceChatBuffer[cg.voiceChatBufferOut].snd) {
			...
			cg.voiceChatTime = cg.time + SECONDS_1;
		}
	}
}
...
void CG_topshotsParse_cmd(qboolean doBest) {
	int Argc = trap_Argc();
	int iArg = 1;
	int iWeap, cnum, hits, atts, kills;
	...
	while( iArg+1 < Argc ) {
		...
		if(ts->cWeapons < WS_MAX * 2) {
			...
			Q_strncpyz(ts->strWS[ts->cWeapons++],
						va("%-12s %5.1f %4d/%-4d %5d  %s", WeaponStatsName(iWeap), acc, hits, atts, kills, name),
						sizeof(ts->strWS[0]));
		}
	}
}
...
void CG_parseBestShotsStats_cmd(qboolean doTop, void (txt_dump)(char *)) {
	...
	qboolean fFull = (txt_dump != CG_printWindow);
	int iArg = 1;
    int iWeap, cnum, hits, atts, kills, deaths;
    float acc;
	...
	while( iArg+1 < Argc ) {
		...
		if(fFull) {
			...
			txt_dump(va("^3%-11s ^7%5.1f ^5%4d/%-4d ^2%5d ^1%6d ^7%s\n",
							aWeaponInfo[iWeap].pszName, acc, hits, atts, kills, deaths, name));
		}
		else {
			BG_cleanName(cgs.clientinfo[cnum].name, name, 12, qfalse);
			txt_dump(va("^3%-4s ^7%5.1f ^5%4d/%-4d ^2%3d ^1%3d ^7%s\n",
							aWeaponInfo[iWeap].pszCode, acc, hits, atts, kills, deaths, name));
		}
	}
}
...
process a "gs" server command
=================
*/
void CG_ServerCommand_gs( void ) {
	int argc = trap_Argc();
	int arg;
#ifdef _DEBUG
...
static void CG_ServerCommand( void ) {
	...
	switch( cmdHash ) {
		...
		case REQFORCESPAWN_HASH:
			...
			if( cg_instanttapout.integer ) {
				...
				if( cgs.gametype == GT_WOLF_LMS ) {
					...
					// core: commented out this menu-message displaying stuff..
					// Just handle it as if the user always has the tapout cvar set.
					// The message was displaying even while the server had no maxlives set at all,
					// but when there is no free spawn found
					// Irata: just do CG_forceTapOut_f for non LMS case.
					// No one will play LMS on a busy 40 slot public server so we shouldn't break LMS here
		...
		case ENTNFO2_HASH:
			{
				...
				CG_ParseMapEntityInfo2( axis_number, allied_number );
			}
			return;
		...
		case CHAT_HASH:
			{
				...
				CG_AddToTeamChat( text, atoi( CG_Argv( 2 ) ), qfalse );
				CG_Printf( "%s\n", text );
				CG_WriteToLog( "%s\n", text );
			}
			return;
		...
		case COMPLAINT_HASH:
			...
			cgs.complaintEndTime = cg.time + SECONDS_20;
			cgs.complaintClient = atoi( CG_Argv(1) );
			if ( cgs.complaintClient < 0 ) {
				cgs.complaintEndTime = cg.time + SECONDS_10;
			}
			return;

		case MAP_RESTART_HASH:
			CG_MapRestart();
			return;
		...
		case PORTALCAMPOS_HASH:
			CG_ParsePortalPos();
			return;

		case SETSPAWNPT_HASH:
			cg.selectedSpawnPoint = atoi(CG_Argv(1)) + 1;
			return;

		case ROCKANDROLL_HASH:
			trap_S_FadeAllSound(1.0f, SECONDS_1, qfalse);	// fade sound up
			return;

		case APPLICATION_HASH:
			cgs.applicationEndTime = cg.time + SECONDS_20;
			cgs.applicationClient = atoi( CG_Argv(1) );
			if ( cgs.applicationClient < 0 ) {
				cgs.applicationEndTime = cg.time + SECONDS_10;
			}
			return;

		case INVITATION_HASH:
			cgs.invitationEndTime = cg.time + SECONDS_20;
			cgs.invitationClient = atoi( CG_Argv(1) );
			if ( cgs.invitationClient < 0 ) {
				cgs.invitationEndTime = cg.time + SECONDS_10;
			}
			return;

		case PROPOSITION_HASH:
			cgs.propositionEndTime = cg.time + SECONDS_20;
			cgs.propositionClient = atoi( CG_Argv(1) );
			cgs.propositionClient2 = atoi( CG_Argv(2) );
			if ( cgs.propositionClient < 0 ) {
				cgs.propositionEndTime = cg.time + SECONDS_10;
			}
			return;

		case AFT_HASH:
			cgs.autoFireteamEndTime = cg.time + SECONDS_20;
			cgs.autoFireteamNum = atoi( CG_Argv(1) );
			if( cgs.autoFireteamNum < -1 ) {
				cgs.autoFireteamEndTime = cg.time + SECONDS_10;
			}
			return;

		case AFTC_HASH:
			cgs.autoFireteamCreateEndTime = cg.time + SECONDS_20;
			cgs.autoFireteamCreateNum = atoi( CG_Argv(1) );
			if( cgs.autoFireteamCreateNum < -1 ) {
				cgs.autoFireteamCreateEndTime = cg.time + SECONDS_10;
			}
			return;

		case AFTJ_HASH:
			cgs.autoFireteamJoinEndTime = cg.time + SECONDS_20;
			cgs.autoFireteamJoinNum = atoi( CG_Argv(1) );
			if( cgs.autoFireteamJoinNum < -1 ) {
				cgs.autoFireteamJoinEndTime = cg.time + SECONDS_10;
			}
			return;
