...
// #define SOUND_FOCUS		SOUNDEVENT( cgs.media.sndLimboFocus ) // IRATA: we don't use it
#define SOUND_FILTER	SOUNDEVENT( cgs.media.sndLimboFilter )
// #define SOUND_CANCEL	SOUNDEVENT( cgs.media.sndLimboCancel ) // blehhh

void CG_DrawBorder( float x, float y, float w, float h, qboolean fill, qboolean drawMouseOver );
int CG_LimboPanel_GetBestSecondaryWeaponNum( int playerClass );
...
void CG_LimboPanel_SendSetupMsg( qboolean forceteam ) {
	...
	CG_PriorityCenterPrint(
		va( "You will spawn as an %s %s with a %s and %s.",
			str, BG_ClassnameForNumber( CG_LimboPanel_GetClass() ),
			BG_Weapons[weap1].name, BG_Weapons[weap2].name ),
			SCREEN_HEIGHT - 88,
			SMALLCHAR_WIDTH,
			-1 );
...
qboolean CG_LimboPanel_ClassIsDisabled( int index);
...
qboolean CG_LimboPanel_ClassButton_KeyDown( panel_button_t* button, int key ) {
	...
	if( key == K_MOUSE1 ) {
		...
		if( cgs.ccSelectedClass != button->data[1] && !CG_LimboPanel_ClassIsDisabled(button->data[1])  ) {
			cgs.ccSelectedClass = button->data[1];

			CG_LimboPanel_SetSelectedWeaponNumForSlot( 0, 0 );
			cgs.ccSelectedWeapon2 = CG_LimboPanel_GetBestSecondaryWeaponNum( cgs.ccSelectedClass );
			trap_Cvar_Set( "cg_limbo_secondary", va("%i", cgs.ccSelectedWeapon2) );
...
void CG_LimboPanel_RenderClassButton( panel_button_t* button ) {
	...
	for( i = 0; i < NUM_SKILL_LEVELS-1; i++ ) {
		...
		if( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR ) {
				CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, cgs.media.limboClassButton2Wedge_off[i] );
		}
		else {
			if( button->data[1] == CG_LimboPanel_GetClass() ) {
				CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, cgs.media.limboClassButton2Wedge_on[i] );
			}
			else if( BG_CursorInRect( &button->rect ) ) {
	...
	if ( CG_LimboPanel_GetTeam() != TEAM_SPECTATOR && CG_LimboPanel_ClassIsDisabled( button->data[1] )) {
	...
	if( cg_gameType.integer == GT_WOLF_LMS ) {
		...
		if( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR ) {
			...
			if( cgs.ccSelectedObjective != CG_LimboPanel_GetMaxObjectives() ) {
				const char* cs = CG_ConfigString( CS_MULTI_OBJECTIVE );
				char *info;
...
void CG_DrawLimboPlayerHead( rectDef_t *rect, bg_character_t* character, float yaw, float pitch, hudHeadAnimNumber_t animation, int team, int cls, int rank, qboolean armor ) {
	float			len;
	vec3_t			origin;
	...
	if ( rank >= NUM_EXPERIENCE_LEVELS-1 && !armor ) {
		hatIndex = ACC_MOUTH2;
	}
	...
	CG_HudHeadAnimation( character, &cg.predictedPlayerEntity.pe.hudhead, &head.oldframe, &head.frame, &head.backlerp, animation, NULL );
...
void CG_LimboPanel_RenderHead( panel_button_t* button ) {
	...
	if( CG_LimboPanel_GetTeam() != TEAM_SPECTATOR ) {
		qboolean armor = qfalse;

		if ( CG_LimboPanel_GetClass() == PC_SOLDIER && cgs.clientinfo[cg.clientNum].skillBits[SK_SOLDIER] & (1<<9) ) {
			armor = qtrue;
		}
		CG_FillRect( button->rect.x, button->rect.y, button->rect.w, button->rect.h, clrBack );
		CG_DrawLimboPlayerHead( &button->rect, CG_LimboPanel_GetCharacter(), 180, 0, HD_IDLE4, CG_LimboPanel_GetTeam(), CG_LimboPanel_GetClass(), cgs.clientinfo[cg.clientNum].rank, armor );
	}
	else {
...
void CG_LimboPanel_WeaponPanel_DrawWeapon( rectDef_t* rect, weapon_t weap, qboolean highlight, const char* ofTxt, qboolean disabled ) {
	...
	if( highlight && BG_CursorInRect( rect ) ) {
		...
		clr[3] *= 1.5;
		CG_Text_Paint_Ext( rect->x + 4, rect->y + 12, weaponPanelNameFont.scalex, weaponPanelNameFont.scaley, clr, BG_Weapons[weap].name, 0, 0, weaponPanelNameFont.style, weaponPanelNameFont.font );
	}
	else {
		CG_Text_Paint_Ext( rect->x + 4, rect->y + 12, weaponPanelNameFont.scalex, weaponPanelNameFont.scaley, weaponPanelNameFont.colour, BG_Weapons[weap].name, 0, 0, weaponPanelNameFont.style, weaponPanelNameFont.font );
	}
...
void CG_LimboPanel_WeaponPanel( panel_button_t* button ) {
	...
	if( BG_PanelButtons_GetFocusButton() == button && cnt > 1 ) {
...

		{
			...
			for( i = 0, x = 1; i < cnt; i++ ) {
// TODO: new restrictions
				cycleWeap = CG_LimboPanel_GetWeaponForNumber( i, cgs.ccSelectedWeaponNumber, qtrue );
				if( cycleWeap != weap && (cycleWeap != WP_VENOM || (cycleWeap == WP_VENOM && !weapon_restrictions[WP_VENOM].disabled)) ) {
					CG_LimboPanel_WeaponPanel_DrawWeapon( &rect, cycleWeap, qtrue, va( "%iof%i", i+1, cnt ), CG_LimboPanel_RealWeaponIsDisabled( cycleWeap ) );

					if( BG_CursorInRect( &rect ) ) {
						if( button->data[7] != x ) {
							// IRATA we don't use it
							// SOUND_FOCUS;
	...
	switch( button->data[0] ) {
		case 0: // class counts
			...
			for( i = 0; i < cgs.numValidClients; ++i ) {
				...
				if( cgs.clientinfo[clientNum].team != CG_LimboPanel_GetTeam() || cgs.clientinfo[clientNum].cls != button->data[1]) {
					continue;
				}
				count++;
			}
		...
		case 1: // team counts
			for( i = 0; i < cgs.numValidClients; ++i ) {
				clientNum = cgs.validClients[i];
				if( cgs.clientinfo[clientNum].team != teamOrder[button->data[1]] ) {
					continue;
				}
				count++;
			}
		...
		case 5: // clock
			...
			count = (( cgs.timelimit * 60 * SECONDS_1 ) - ( cg.time - cgs.levelStartTime )) / SECONDS_1;
			switch( button->data[1] ) {
				case 0: // secs
					return count % 60;
				case 1: // mins
...
// IRATA: let's return float!
float CG_LimboPanel_RenderCounter_RollTimeForButton( panel_button_t* button ) {
	switch( button->data[0] ) {
		case 0: // class counts
		case 1: // team counts
			return 50.f; // IRATA: was 100.f ... lets do it faster ... this is important to know
		...
		case 6: // stats
		{
			float diff = Q_fabs( button->data[3] - CG_LimboPanel_RenderCounter_ValueForButton( button ));
			if( diff < 5 ) {
				if (diff <= 0) { // IRATA: avoid div/0 - happens quite often for stats - see diff init
					return 200.f; // 200.f / 1
				}
...
qboolean CG_LimboPanel_RenderCounter_StartSet( panel_button_t* button ) {
	switch( button->data[0] ) {
		// case 2: doesn't work cg.xp is not set at start
		case 3: // respawn time
		case 5: // clock
			return qtrue;
...
#define MAX_ROLLERS 9
#define COUNTER_ROLLTOTAL (cg.time - button->data[4])

static qboolean isXPinit = qfalse;
/*
 *
 * Gordon: this function is mental, i love it :)
 * IRATA: love it too ... I would say it's well documented
 *
 *
 * button->data[0] - button type see f.e. CG_LimboPanel_RenderCounter_RollTimeForButton
 * button->data[1]
 * button->data[2]
 * button->data[3] - old value
 * button->data[4] - roll time - see also COUNTER_ROLLTOTAL
 * button->data[5] - new value
 * button->data[6]
 */
void CG_LimboPanel_RenderCounter( panel_button_t* button ) {
	...
	// IRATA: special hack to show higher XP value on fresh connect w/o roller
	// Note: Adding the xp button to CG_LimboPanel_RenderCounter_StartSet
	// for CG_LimboPanel_Setup didn't work at start -> xg.xp is empty ...
	if (button->data[0] == 2 && !isXPinit) {
	...
	//	DEBUG
	//if (button->data[0] == 2 ) {
	//	CG_Printf("CG_LimboPanel_RenderCounter cg.XP: %i button-o: %i button-n: %i\n", cg.xp,  button->data[3] , button->data[5]);
	//}
	...
	if( COUNTER_ROLLTOTAL < counter_rolltime ) {
		...
		if( button->data[3] != button->data[5] ) { // old value != new value - just copy new into old
	...
	if( button->data[0] == 0 || button->data[0] == 1) { // 0=class, 1= team count
...
void CG_LimboPanel_Setup( void ) {
	...
	if( !cgs.limboLoadoutSelected ) {
		...
		cgs.ccSelectedWeapon2 = CG_LimboPanel_GetBestSecondaryWeaponNum( cgs.ccSelectedClass );
		if ( cg_limbo_secondary.integer > 0 && cg_limbo_secondary.integer < CG_LimboPanel_WeaponCount_ForSlot( 0 ) ) {
...
qboolean CG_LimboPanel_Draw( void ) {
	...
	panel_button_t* hilight = BG_PanelButtonsGetHighlightButton( limboPanelButtons );
	...
	if( cgs.ccRequestedObjective != -1 ) {
		if( cg.time - cgs.ccLastObjectiveRequestTime > SECONDS_1 ) {
			if( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR ) {
...
// TODO: rework for new restriction stuff? - if we dont want the weapon card / no disabled banner
//			if ( weapon_restrictions[classInfo->classWeapons[i]].disabled ) continue;
// weapon_t weapon = classinfo->classWeapons[index];
			// dont count venom if disabled
			if ( classInfo->classWeapons[i] == WP_VENOM && weapon_restrictions[WP_VENOM].disabled ) continue;
		...
		if( cgs.clientinfo[cg.clientNum].skillBits[SK_HEAVY_WEAPONS] & (1<<4) && CG_LimboPanel_GetClass() == PC_SOLDIER ) {
			if ( cgs.soldierShotgun ) {
				if( cgs.clientinfo[cg.clientNum].skillBits[SK_LIGHT_WEAPONS] & (1<<4) ) {
					return 4;
				}
				else {
					return 3;
...
int CG_LimboPanel_GetBestSecondaryWeaponNum( int playerClass ) {
	if ( playerClass == PC_SOLDIER && (cgs.clientinfo[cg.clientNum].skillBits[SK_HEAVY_WEAPONS] & (1<<4)) ) {
		if ( cgs.clientinfo[cg.clientNum].skillBits[SK_LIGHT_WEAPONS] & (1<<4) ) {
			return 2; // Default to SMG (0=Pistol, 1=Akimbo, 2=SMG, 3=Shotgun)
		} else {
			return 1; // Default to SMG (0=Pistol, 1=SMG, 2=Shotgun)
		}
	}

	if ( cgs.clientinfo[cg.clientNum].skillBits[SK_LIGHT_WEAPONS] & (1<<4) ) {
		return 1; // Default to Akimbo pistols (0=Pistol, 1=Akimbo)
	}

	return 0; // Default to single pistol
}
...
weapon_t CG_LimboPanel_GetWeaponForNumber( int number, int slot, qboolean ignoreDisabled ) {
	...
	if( slot == 1 ) {
		if( !ignoreDisabled && CG_LimboPanel_WeaponIsDisabled( number ) ) {
			...
			//	return WP_NONE;
				if ( CG_LimboPanel_GetClass() == PC_SOLDIER )
				return CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_STG44 : WP_BAR;
				else if ( CG_LimboPanel_GetClass() == PC_COVERTOPS )
				return CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_MP34 : WP_STEN;
					else
...
void CG_LimboPanel_SetSelectedWeaponNumForSlot( int index, int number ) {
	if( index == 0 ) {
		...
		// IRATA #365: Auto switch bug on respawn / cg_limbo_secondary 2 bug
		// Note for cg_limbo_secondary.integer >= 2
		// - if a soldier player sets his secondary weapon to pistols it will kept as long as the the player doesn't change primary weapons
		//   On primary change it will be set to smg/shotgun again which is correct because cg_limbo_secondary is still >= 2
		if (cg_limbo_secondary.integer >= 2 &&
				cgs.clientinfo[cg.clientNum].skillBits[SK_HEAVY_WEAPONS] & (1<<4) && CG_LimboPanel_GetClass() == PC_SOLDIER) {
			cgs.ccSelectedWeapon2 = cg_limbo_secondary.integer;
		}
	}
	else {
...
void CG_LimboPanel_RequestObjective( void ) {
	...
	if( cgs.ccSelectedObjective != CG_LimboPanel_GetMaxObjectives() && CG_LimboPanel_GetTeam() != TEAM_SPECTATOR ) {
...
int CG_LimboPanel_ClassCount( int cls ) {
	int i, cnt;
	int clientNum;
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( cgs.clientinfo[clientNum].team != CG_LimboPanel_GetTeam() ) {
			continue;
		}

		if( cls != -1 ) {

			if( cgs.clientinfo[clientNum].cls != cls && cgs.clientinfo[clientNum].latchedCls != cls ) {
				continue;
			}
		}
...
/*
 * Counts numbers of weapons in game (not team specific)
 */
int CG_LimboPanel_TeamCount( weapon_t weap ) {
	int i, cnt = (weap == -1)? 1 : 0;
	int clientNum;
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( !(cgs.clientinfo[clientNum].team == TEAM_AXIS || cgs.clientinfo[clientNum].team == TEAM_ALLIES) ) {
			continue;
		}
		...
		if ( cgs.clientinfo[clientNum].weapon == weap || cgs.clientinfo[clientNum].latchedweapon == weap ||
			cgs.clientinfo[clientNum].weapon == weapAlts[weap] || cgs.clientinfo[clientNum].latchedweapon == weapAlts[weap] ) {
			++cnt;
		}
	}
	return cnt;
...
int CG_LimboPanel_TeamCount2( int team ) {
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		clientNum = cgs.validClients[i];
		if( cgs.clientinfo[clientNum].team != team ) {
			continue;
		}
		cnt++;
	}
...
qboolean CG_LimboPanel_ClassIsDisabled( int index ) {
	...
	switch(index) {
		case PC_SOLDIER:
			value = cgs.maxSoldiers;
			break;
		case PC_MEDIC:
			value = cgs.maxMedics;
			break;
		case PC_ENGINEER:
			value = cgs.maxEngineers;
			break;
		case PC_FIELDOPS:
			value = cgs.maxFieldops;
			break;
		case PC_COVERTOPS:
			value = cgs.maxCovertops;
			break;
		default:
			return qfalse;
	}
	...
	count =		CG_LimboPanel_ClassCount( -1 );
	wcount =	CG_LimboPanel_ClassCount( index );
...
// Always syncronize with G_IsWeaponDisabled g_cmds !!!!!!!!! -> doesn't seem to be the case atm
// IRATA: FIXME check this !!! -> panzer was available, but not after spawn !
qboolean CG_LimboPanel_RealWeaponIsDisabled( weapon_t weap ) {
	int numPlayingClients, wcount;

	if( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR ) {
		return qtrue;
	}

	// if the weapon is not restricted, then we're done..
	if ( !weapon_restrictions[weap].restricted ) {
		return qfalse;
	}

	// if the weapon is disabled, then we're done..
	if ( weapon_restrictions[weap].disabled ) {
		return qtrue;
	}

	numPlayingClients = CG_LimboPanel_TeamCount2( CG_LimboPanel_GetTeam() );

	// are there enough players to activate the weapon?..
	if ( weapon_restrictions[weap].r_minplayers && numPlayingClients < weapon_restrictions[weap].minplayers ) {
		return qtrue;
	}

	// counts number of players with specific weapon
	wcount = CG_LimboPanel_TeamCount( weap );

	// are there already  the absolute max. ingame allowed  of the weapon?..
	if ( weapon_restrictions[weap].r_maxweap && wcount >= weapon_restrictions[weap].maxweap ) {
		return qtrue;
	}

	// TEST TEST TEST THIS BEFORE RELEASE ...
	// it's me the player who wants this weapon ... keep in mind we are not in the team at that time
	// (only do this if current team is spec ? - spec is early out here
	if (wcount == 0) {
		wcount++;
	}
// CG_Printf("LIMBO weapon count %d >= %f \n", wcount, numPlayingClients * 0.01f * weapon_restrictions[weap].percentage);

	// is the percentage of players using the weapon not exceeded?..
	if ( weapon_restrictions[weap].r_percentage && wcount >= numPlayingClients * 0.01f * weapon_restrictions[weap].percentage ) {
		return qtrue;
	}
