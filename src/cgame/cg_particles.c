...
// TODO: remove the commented from unsorted shader file
...
void CG_ClearParticles(void) {
	...
	if (!initparticles) { // IRATA: CG_ClearParticles is also called on maprestart - we don't have to register twice!
...
void CG_AddParticleToScene (cparticle_t *p, vec3_t org, float alpha) {
	...
	case P_BUBBLE_TURBULENT:
		// create a front facing polygon
		{
			...
			if(VectorDistanceSquared( cg.snap->ps.origin, org ) > 1048576) {
				return;
			}
			// done.
	...
	case P_SPRITE:
		{
			...
			s = 2*height;
			if (p->roll) {
				VectorMA (point, s, ru, point);
			}
			else {
				VectorMA (point, s, vup, point);
			}
			VectorCopy (point, verts[1].xyz);
			verts[1].st[0] = 0;
			verts[1].st[1] = 1;
			...
			s = 2*width;
			if (p->roll) {
				VectorMA (point, s, rr, point);
			}
			else {
				VectorMA (point, s, vright, point);
			}
			VectorCopy (point, verts[2].xyz);
			verts[2].st[0] = 1;
			verts[2].st[1] = 1;
	...
	case P_SMOKE_IMPACT:
		// create a front rotating facing polygon
		{
			...
			if ( p->type == P_SMOKE_IMPACT && VectorDistanceSquared( cg.snap->ps.origin, org ) > 1048576) {
				return;
			}
...
void CG_ParticleSmoke (qhandle_t pshader, centity_t *cent) {
	...
	if (!pshader) {
		pshader = cgs.media.smokePuffShader;
	}

	if (!pshader || !free_particles) {
		return;
	}
	p = free_particles;
	free_particles = p->next;
...
// IRATA: FIXME/inspect ... r = 2 has no shader
void CG_ParticleDirtBulletDebris (vec3_t org, vec3_t vel, int duration) {
	...
	if ( r == 0 ) {
		...
		p->pshader = 0; // cgs.media.dirtParticle3Shader; // IRATA: not init/defined/in path
	}
...
void CG_ParticleImpactSmokePuff (qhandle_t pshader, vec3_t origin) {
	if (cg_smokeparticles.integer) {
		CG_ParticleImpactSmokePuffExtended (pshader, origin, SECOND_HALF, 20, 20, 30, 0.25f, 8.f);
	}
}
...
#define EXTRUDE_DIST	0.5
	...
	for (x= -fwidth/2; x<fwidth; x+= fwidth) {
		...
		for (y= -fheight/2; y<fheight; y+= fheight) {
			...
			s = -EXTRUDE_DIST*2;
			VectorMA (this_pos, s, normal, end_pos);
...
void CG_ParticleBloodCloudZombie (centity_t *cent, vec3_t origin, vec3_t dir) {
	...
	for (i=0; i<dist; ++i) {
		...
		if (length) {
			p->endtime = cg.time + 3500 + (crandom() * SECONDS_2);
		}
		else {
