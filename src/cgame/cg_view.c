...
static int CG_CalcFov( void ) {
	...
	// MODERN WIDESCREEN FIX: Force Hor+ scaling (Widen the sides, keep vertical height locked)
	{
		float aspect = (float)cgs.glconfig.vidWidth / (float)cgs.glconfig.vidHeight;

		// 1. Figure out what the vertical FOV *should* be on a classic 4:3 monitor
		fov_y = atan2(240.0f, 320.0f / tan(fov_x / 360.0f * M_PI)) * 360.0f / M_PI;

		// 2. Expand the horizontal FOV to perfectly fill your widescreen monitor
		fov_x = atan(aspect * tan(fov_y / 360.0f * M_PI)) * 360.0f / M_PI;
	}
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION && cgs.mvAllowed ) {
		...
		if ( !cg.showGameView && !cg.intermissionStarted && !cgs.dbShowing ) {
			...
			// core: The killer Cam..
				...
				int kNum = cg.latestKiller->currentState.number;
				if ( kNum >= 0 && kNum < MAX_CLIENTS && cgs.clientinfo[ kNum ].infoValid ) {
					// IRATA: print the name of our killer instead of "KILLER CAM" ... 20 chars ...
					CG_DrawStringExt( 16, 145, va("^W%s^W", cgs.clientinfo[ kNum ].name), colorWhite, qfalse, qtrue, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 20);
