...
qboolean CG_SpawnFloat( const char *key, const char *defaultString, float *out ) {
	char		*s;
	qboolean	present = CG_SpawnString( key, defaultString, &s );
	*out = atof( s );
	return present;
}
...
void SP_misc_gamemodel( void ) {
	...
	if (gamemodel->model == 0) { // IRATA: don't register game models twice (f.e. after warmup or 'restart')
		gamemodel->model = trap_R_RegisterModel( model );
	}
	AnglesToAxis( angles, gamemodel->axes );

	VectorScale( gamemodel->axes[0], vScale[0], gamemodel->axes[0] );
	VectorScale( gamemodel->axes[1], vScale[1], gamemodel->axes[1] );
	VectorScale( gamemodel->axes[2], vScale[2], gamemodel->axes[2] );
...
void SP_trigger_objective_info( void ) {
	...
	// Bounds check: custom maps may have more trigger_objective_info entities than MAX_OID_TRIGGERS
	if ( cg.numOIDtriggers2 >= MAX_OID_TRIGGERS ) {
		CG_Printf( "^3WARNING: SP_trigger_objective_info: MAX_OID_TRIGGERS (%i) exceeded, ignoring extra entry\n", MAX_OID_TRIGGERS );
		return;
	}
...
// CHRUKER: b043 - Make sure the maximum commandmaps, doesn't overflow the maximum allowed command map layers if (cgs.ccLayers > MAX_COMMANDMAP_LAYERS) { cgs.ccLayers = MAX_COMMANDMAP_LAYERS; CG_Printf( va("^3Warning: The maximum number (%i) of command map layers is exceeded.\n", MAX_COMMANDMAP_LAYERS) ); }
	...
	cg.fiveMinuteSound_g[0]   = '\0';
	cg.fiveMinuteSound_a[0]	  = '\0';
	cg.twoMinuteSound_g[0]	  = '\0';
	cg.twoMinuteSound_a[0]	  = '\0';
	cg.thirtySecondSound_g[0] = '\0';
