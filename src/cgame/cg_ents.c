...
void CG_SetEntitySoundPosition( centity_t *cent ) {
	if ( cent->currentState.solid == SOLID_BMODEL ) {
		vec3_t	origin;
		const float	*v = (cent->currentState.modelindex >= 0 && cent->currentState.modelindex < MAX_MODELS) ?
			cgs.inlineModelMidpoints[ cent->currentState.modelindex ] : vec3_origin;
...
static void CG_EntityEffects( centity_t *cent ) {
	...
	// jaquboss so we can aquire some free eflags
	...
	if (CG_EntOnFire(cent)) {
		...
		// IRATA: FIXME - this doesn't work anymore for some reasons ...
		trap_S_AddLoopingSound( cent->lerpOrigin, vec3_origin, cgs.media.flameBlowSound, (int)(255.0f*(1.0f-fabs(cent->fireRiseDir[2]))), 0 );
		// play a burning sound when not moving
		trap_S_AddLoopingSound( cent->lerpOrigin, vec3_origin, cgs.media.flameSound, (int)(0.3f*255.0f*(pow(cent->fireRiseDir[2],2))), 0 );
	}

	// ydnar: overheating is a special effect
	if( (cent->currentState.eFlags & EF_OVERHEATING) == EF_OVERHEATING && cg_smokeparticles.integer ) {
		if( cent->overheatTime < (cg.time - SECONDS_3) ) {
...
// JPW NERVE -- use wind vector for smoke
			...
			CG_SmokePuff( cent->lerpOrigin, dir, 15+(random()*10),
					...
					cg.time, cg.time + SECOND_HALF, 0, cgs.media.smokePuffShader);
		}
	}
	// dhm - end
...
static void CG_General( centity_t *cent ) {
	...
	ent.hModel = CG_GetGameModel(s1->modelindex);
...
qboolean CG_PlayerSeesItem(playerState_t *ps, entityState_t *item, int atTime, int itemType) {
	...
	if(dot > (-0.94f - (dist*(1.0f/255.0f)) * 0.057f) ) {
		return qfalse;
	}
...
static void CG_Item( centity_t *cent ) {
	...
	// core: first check for dogtags because it has no weaponInfo..
	if ( item->giType == IT_WEAPON && item->giTag == ITEM_DOGTAGS ) {
		AnglesToAxis( cent->lerpAngles, ent.axis );
		VectorCopy( cent->lerpOrigin, ent.origin);
		VectorCopy( cent->lerpOrigin, ent.oldorigin);
		ent.hModel = cgs.cachedModels[GAMEMODEL_WORLD_DOGTAGS];
	}
	else {
		...
		if(item->giType == IT_WEAPON) {
			...
			if(hasStand) {							// first try to put the weapon on it's 'stand'
				...
				if( weaponInfo->droppedAnglesHack && ( cent->currentState.pos.trType == TR_STATIONARY || cent->currentState.pos.trType == TR_GRAVITY_PAUSED  ) ) {
		...
		if(es->modelindex2) {	// modelindex2 was specified for the ent, meaning it probably has an alternate model (as opposed to the one in the itemlist)
			...
			ent.hModel = CG_GetGameModel( es->modelindex2 );
		}
		else {
			if( item->giType == IT_WEAPON || item->giTag == WP_MEDKIT ) {

				if ( es->teamNum == TEAM_AXIS && cg_weapons[item->giTag].weaponModel[W_PU_MODEL].model[TEAM_AXIS] )
				ent.hModel = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].model[TEAM_AXIS];
					else if ( es->teamNum == TEAM_ALLIES && cg_weapons[item->giTag].weaponModel[W_PU_MODEL].model[TEAM_ALLIES] )
				ent.hModel = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].model[TEAM_ALLIES];
					else
				ent.hModel = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].model[0];


				if ( es->teamNum == TEAM_AXIS && cg_weapons[item->giTag].weaponModel[W_PU_MODEL].skin[TEAM_AXIS] )
				ent.customSkin = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].skin[TEAM_AXIS];
					else if ( es->teamNum == TEAM_ALLIES && cg_weapons[item->giTag].weaponModel[W_PU_MODEL].skin[TEAM_ALLIES] )
				ent.customSkin = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].skin[TEAM_ALLIES];
					else
				ent.customSkin = cg_weapons[item->giTag].weaponModel[W_PU_MODEL].skin[0];

				if( item->giTag == WP_AMMO ) {
					if( cent->currentState.density == 3 ) {
						ent.customShader = cg_weapons[item->giTag].modModels[1];
					} else if( cent->currentState.density == 2 ) {
						ent.customShader = cg_weapons[item->giTag].modModels[0];
					}
				}
			}
			else if ( item->giType == IT_HEALTH || item->giType == IT_AMMO ) {
				if ( es->density < (1 << 9) ) {
				...
				if ( item->giTag == PW_HELMETSHIELD || item->giTag == PW_HELMET ) {
					bg_character_t	*character=CG_CharacterForPlayerstate( &cg.snap->ps );

					if ( character ) {
						ent.hModel = item->giTag == PW_HELMETSHIELD ? character->accModels[ACC_SHIELD] : character->accModels[ACC_HAT];
						ent.customSkin = item->giTag == PW_HELMETSHIELD ? character->accSkins[ACC_SHIELD] : character->accSkins[ACC_HAT];
						VectorMA(ent.origin, -4.0f, ent.axis[2], ent.origin );
					}
					else {
						ent.hModel = cg_items[es->modelindex].models[0];
					}
...
//	}
	...
	if(cg_drawCrosshairPickups.integer) {
		...
		if ( highlight ) {
			...
			ent.hilightIntensity = 1.0f - ( ( cg.time - cent->highlightTime ) / SECONDS_1_F ) * highlightFadeScale;	// 1 sec to dim down (diff in time causes problems if you quickly flip to/away from looking at the item)
		}
...
static void CG_Smoker( centity_t *cent ) {
	if( cg.time - cent->highlightTime > cent->currentState.constantLight ) {
		...
		if(cent->currentState.modelindex2 > 0 && cent->currentState.modelindex2 < MAX_CS_SHADERS && cgs.gameShaders[cent->currentState.modelindex2]) {
...
static void CG_DrawMineMarkerFlag( centity_t *cent, refEntity_t *ent, const weaponInfo_t *weapon ) {
	...
	if( cg.time >= cent->lerpFrame.frameTime ) {
		...
		while( cg.time >= cent->lerpFrame.frameTime ) {
			cent->lerpFrame.frameTime += SERVER_FRAMETIME;	// 1000 / fps (which is 20)
			cent->lerpFrame.frame++;
...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	if ( weapon->missileSound ) {
		if( IS_RIFLENADE_WEAPON(cent->currentState.weapon) ) {
	...
	if ( cent->currentState.weapon == WP_DYNAMITE ) {
		...
		if ( cent->currentState.teamNum % 4 == cgs.clientinfo[cg.snap->ps.clientNum].team ||
	...
	if (cent->currentState.eType == ET_FP_PARTS) {
		ent.hModel = CG_GetGameModel(cent->currentState.modelindex);
	}
		...
		if ( cent->currentState.weapon == WP_LANDMINE || cent->currentState.weapon == WP_DYNAMITE ){
			missileTeam = cent->currentState.teamNum % 4;
		}
		else {
			missileTeam = cent->currentState.teamNum;
		}

		ent.hModel = weapon->missileModel[0];

		if( missileTeam == TEAM_ALLIES ) {
			ent.customSkin = weapon->missileAlliedSkin;

			if ( weapon->alliedMissileModel ){
				ent.hModel = weapon->missileModel[TEAM_ALLIES];
			}
		}
		else if( missileTeam == TEAM_AXIS ) {
			ent.customSkin = weapon->missileAxisSkin;

			if ( weapon->axiesMissileModel ){
				ent.hModel = weapon->missileModel[TEAM_AXIS];
			}
		}
	}
	...
	if( cent->currentState.weapon == WP_LANDMINE ) {
		if( cgs.clientinfo[ cg.snap->ps.clientNum ].team == TEAM_SPECTATOR ) { // jaquboss, draw a mine for followed clients ( ie draw what the player we follow see )
			return;
		}
		...
		if(cent->currentState.teamNum < 4) {
			...
			if((cgs.clientinfo[cg.snap->ps.clientNum].team != (!cent->currentState.otherEntityNum2 ? TEAM_ALLIES : TEAM_AXIS))) {
				...
				else {// it's already spotted

					CG_DrawMineMarkerFlag( cent, &ent, weapon );
				}

			}
			else { // it's on our team, we can see it

				// jet Pilot - thanks to forty - mine id
	...
	if( IS_MORTAR_WEAPON_SET(cent->currentState.weapon) ||
		IS_RIFLENADE_WEAPON(cent->currentState.weapon)  ||
		IS_PANZER_WEAPON(cent->currentState.weapon) ) {
        vec3_t delta;
	...
	if ( cent->currentState.clientNum == cg.predictedPlayerState.clientNum && cg.snap->ps.clientNum == cg.clientNum ) {
		if ( cg_insanity.integer & JP_INSANITY_MISSILECAM && IS_PANZER_WEAPON(s1->weapon) ) {
		...
		else if ( (cg_insanity.integer & JP_INSANITY_RIFLENADECAM) && !(cg.snap->ps.pm_flags & PMF_FOLLOW) && IS_RIFLENADE_WEAPON(s1->weapon) ) {
			cg.latestMissile = cent;
		}
		else if ( cg_insanity.integer & JP_INSANITY_MORTARCAM && IS_MORTAR_WEAPON(s1->weapon) ) {
			cg.latestMissile = cent;
		}
	}
...
==============
*/
static void CG_PitfallSetAnim(centity_t *cent, lerpFrame_t *lf, int newAnim) {
...
==============
*/
static void CG_Pitfall(centity_t *cent) {
	refEntity_t			ent;
	...
	if(!traplf->oldFrameTime) {
		...
		CG_PitfallSetAnim(cent, traplf, cs->frame);
	...
	ent.hModel = CG_GetGameModel(cs->modelindex);
...
static void CG_Explosive( centity_t *cent ) {
	...
	if ( s1->modelindex2 ) {
		ent.skinNum = 0;
		ent.hModel = CG_GetGameModel(s1->modelindex2);
		trap_R_AddRefEntityToScene(&ent);
	}
	else
		trap_R_AddRefEntityToScene(&ent);
...
void CG_Mover_PostProcess( centity_t* cent ) {
	...
	if( cg.snap->ps.eFlags & EF_MOUNTEDTANK && cg_entities[cg.snap->ps.clientNum].tagParent == cent->currentState.effect3Time ) {
		...
		for( i = 0; i < cgs.numValidClients; ++i ) {
			...
			if( cg_entities[clientNum].currentState.eFlags & EF_MOUNTEDTANK ) {
				if( cg_entities[clientNum].tagParent == cent->currentState.effect3Time ) {
					break;
				}
			}
		}
	}
	if(i != cgs.numValidClients) {
		if( clientNum != cg.snap->ps.clientNum ) {
			angles = cg_entities[clientNum].lerpAngles;
		}
		else {
	...
	if ( cent->currentState.effect3Time >= 0 && cent->currentState.effect3Time < MAX_GENTITIES ) {
		cg_entities[cent->currentState.effect3Time].tankparent = (int)(cent-cg_entities);
		...
		if( cg.snap->ps.eFlags & EF_MOUNTEDTANK && cg.snap->ps.clientNum >= 0 && cg.snap->ps.clientNum < MAX_CLIENTS ) {
			int tagParent = cg_entities[cg.snap->ps.clientNum].tagParent;
			if ( tagParent >= 0 && tagParent < MAX_GENTITIES ) {
				centity_t* tank = &cg_entities[tagParent];
...
static void CG_Portal( centity_t *cent ) {
	...
	VectorCopy( cent->lerpOrigin, ent.origin );
	VectorCopy( s1->origin2, ent.oldorigin );
	ByteToDir2( s1->eventParm, ent.axis[0] );
...
void CGRefEntityToTag( refEntity_t* ent, tag_t* tag ) {
	...
	VectorCopy(ent->axis[0], tag->axis[0]);
	VectorCopy(ent->axis[1], tag->axis[1]);
	VectorCopy(ent->axis[2], tag->axis[2]);
}

void CGTagToRefEntity( refEntity_t* ent, tag_t* tag ) {
	VectorCopy( tag->origin, ent->origin );

	VectorCopy(tag->axis[0], ent->axis[0]);
	VectorCopy(tag->axis[1], ent->axis[1]);
	VectorCopy(tag->axis[2], ent->axis[2]);
}
...
void CG_AttachBitsToTank( centity_t* tank, refEntity_t* mg42base, refEntity_t* mg42upper, refEntity_t* mg42gun, refEntity_t* player, refEntity_t* flash, vec_t* playerangles, const char* tagName, qboolean browning ) {
	...
	if( tank->tankframe != cg.clientFrame ) {
		...
		if ( tank->currentState.solid == SOLID_BMODEL ) {
			ent.hModel = CG_GetGameModel(tank->currentState.modelindex2);
		}
		else {
			ent.hModel = CG_GetGameModel(tank->currentState.modelindex);
		}
		...
		if( cg.snap->ps.eFlags & EF_MOUNTEDTANK
			...
			for( i = 0; i < cgs.numValidClients; ++i ) {
				clientNum = cgs.validClients[i];
				// Gordon: is this entity mounted on a tank, and attached to _OUR_ turret entity (which could be us)
				if( cg_entities[clientNum].currentState.eFlags & EF_MOUNTEDTANK && cg_entities[clientNum].tagParent == tank - cg_entities ) {
					angles[YAW] -= tank->lerpAngles[YAW];
					angles[PITCH] -= tank->lerpAngles[PITCH];
					break;
				}
