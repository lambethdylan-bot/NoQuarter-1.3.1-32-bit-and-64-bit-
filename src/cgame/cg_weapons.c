...
#define WEAPONCYCLEDELAY 150 // (3*SERVER_FRAMETIME) - was cg_weaponcycledelay before

// IRATA moved from bg back to cg_weapons
// JPW NERVE
...
int weapBanksMultiPlayer[MAX_WEAP_BANKS_MP][MAX_WEAPS_IN_BANK_MP] = {
	...
	{WP_MP40,				WP_THOMPSON,			WP_STEN,				WP_GARAND,				WP_PANZERFAUST,
	 ...
	 WP_MOBILE_MG42,		WP_MORTAR,				WP_SHOTGUN,				WP_MOBILE_BROWNING,		WP_JOHNSON,
...
void CG_MachineGunEjectBrassNew( centity_t *cent ) {
	...
	if ( weapon->brassModel )
		...
		case WP_BAR:
		case WP_BAR_SET:
		case WP_KAR98: // machine gun brass ?
		case WP_CARBINE:
		case WP_K43:
		case WP_GARAND:
		case WP_FG42:
		...
		case WP_K43_SCOPE:
		case WP_GARAND_SCOPE: // machine gun brass ? end
		case WP_STG44:
		case WP_VENOM:
			re->hModel = cgs.media.machinegunBrassModel;
			break;
/* IRATA: there is no need to have these cases in the code, there are default anyway ... I'll keep them as docu
		case WP_LUGER:
		case WP_COLT:
		case WP_SILENCER:
...
void CG_PyroSmokeTrail( centity_t *ent, const weaponInfo_t *wi ) {
	...
	if( ent->currentState.weapon == WP_LANDMINE ) {
		...
		if( cg.time - ent->miscTime > SECONDS_1 ) {
			return;
		}
...
/*
==========================
CG_RocketTrail - Ridah, new trail effects
==========================
*/
...
void CG_DynamiteTrail( centity_t *ent, const weaponInfo_t *wi ) {
	...
	if ( ent->currentState.teamNum < 4 ) {
		mult = 0.004f*(cg.time - ent->currentState.effect1Time)/SECONDS_30_F;
...
void CG_RailTrail2( vec3_t color, vec3_t start, vec3_t end, int index, int sideNum ) {
	...
	le->endTime = cg.time + SERVER_FRAMETIME;	// le->endTime = cg.time + cg_railTrailTime.value; // (vanilla cg_railTrailTime = 400)
...
qboolean CG_ParseWeaponConfig( const char *filename, weaponInfo_t *wi ) {
	...
	for ( i = 0 ; i < MAX_WP_ANIMATIONS ; ++i ) {
		...
		wi->weapAnimations[i].frameLerp = SECONDS_1 / fps;
		wi->weapAnimations[i].initialLerp = SECONDS_1 / fps;
...
qboolean CG_RW_ParseModModel( int handle, weaponInfo_t *weaponInfo ) {
	...
	if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
		...
		// new: maybe it's a shader ...
		// IRATA: each trapRegisterModel with non valid result will reserve 128 bytes
		//
		if ( !strstr( filename, ".md3" ) && !strstr( filename, ".mdc" ) ) { // todo: support more model formats ?
		...
		if( !weaponInfo->modModels[mod] ) {
			// still no q_handle_t ... print it
			Com_Printf( S_COLOR_RED "ERROR: CG_RW_ParseModModel() no model or shader for %s registered.\n", filename);
		}
/*
		// IRATA: at this point we don't know if model is a real model or a shader
		// DEBUG build will nag with warning of registering models ...
		// clean solution: check if file is on path ... in case of register
		weaponInfo->modModels[mod] = trap_R_RegisterModel( filename );
		if( !weaponInfo->modModels[mod] ) {
			// maybe it's a shader

			weaponInfo->modModels[mod] = trap_R_RegisterShader( filename );
			// CG_Printf("CG_RW_ParseModModel() no mod model for %s ... trying to register shader instead\n", filename);
			if( !weaponInfo->modModels[mod] ) {
				// still no q_handle_t ... print it
				Com_Printf( S_COLOR_RED "ERROR: CG_RW_ParseModModel() no mod shader for %s ... \n", filename);
			}
		}
...
void CG_RegisterItemVisuals( int itemNum ) {
	...
	for( i = 0; i < MAX_ITEM_MODELS; ++i ) {
		// IRATA: don't display NULL output in xreal ...
		// we don't register if there is no world_model for, to quote Ridah in GPL
		...
		if( item->world_model[i] ) {
			...
			CG_Printf("CG_RegisterItemVisuals(): trap_R_RegisterModel() skipped - world_model[%i] of item %s is empty.\n", i ,item->classname);
		}
#endif		
	}
	...
	if( item->icon && item->icon[0]) {
		...
		CG_Printf("CG_RegisterItemVisuals(): trap_R_RegisterShader() skipped for item %s\n", item->classname);
	}
#endif
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
	...
	if( !cg.renderingThirdPerson && IS_SET_WEAPON(cg.predictedPlayerState.weapon)  &&
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups, playerState_t *ps, centity_t *cent ) {
	refEntity_t backupRefEnt = *gun;//, parentEnt;
	...
	if ( cent->currentState.clientNum >= 0 && cent->currentState.clientNum < MAX_CLIENTS
        ...
        && ((cg.clientNum >= 0 && cg.clientNum < MAX_CLIENTS && cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR
        ...
        || cent->currentState.eFlags & EF_GLOW ))
	{
		...
		if( cent->currentState.powerups & (1 << PW_OPS_DISGUISED) ) {
			if ( cg.clientNum >= 0 && cg.clientNum < MAX_CLIENTS && (cgs.clientinfo[cg.clientNum].team != TEAM_SPECTATOR || !(cgs.spectator  & SPEC_PLAYERGLOW)) ) {
...
Used for both the view weapon (ps is valid - see cg_players) and the world model other character models (ps is NULL - see cg_weapons)
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent) {
	...
	// no weapon when mounted on a tank
	// IRATA: this is 3rd p-view only !!! - see if statement below !ps
	...
	// IRATA: only stationary MGs & only called by !ps -- cg_weapons does early return on ps->eFlags & EF_MG42_ACTIVE
	else if( cent->currentState.eFlags & EF_MG42_ACTIVE) {	// Arnout: MG42 Muzzle Flash
		// IRATA: #603 added cg_drawMuzzleFlash ( 1st p only ...)
		if (isPlayer && !cg.renderingThirdPerson) {
			if (cg_drawMuzzleFlash.integer & 1 && cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME) {
				CG_MG42EFX( cent );
			}
		}
		else {
			...
			if( !IS_PANZER_WEAPON(weaponNum) ) {
				...
				for ( i=0; i < (sizeof(weapOverheat) / sizeof(weapOverheat[0])); ++i ) {
					if ( cent->currentState.weapon == weapOverheat[i] ) {
						if(cg.time - cent->overheatTime < SECONDS_3) {
							if(!(rand()%3)) {
								float alpha;
								alpha = 1.0f - ((float)(cg.time - cent->overheatTime)/SECONDS_3_F);
								alpha *= 0.25f;		// .25 max alpha
								CG_ParticleImpactSmokePuffExtended(
									cgs.media.smokeParticleShader, flash.origin, SECONDS_1, 8, 20, 30, alpha, 8.f);
							}
						}
						break;
					}
				...
				if (cg.time - cent->muzzleFlashTime < SECONDS_1 /*BARREL_SMOKE_TIME*/) { // #define BARREL_SMOKE_TIME 1000	// continuous smoke after firing
					if(!(rand()%5)) {
						float alpha;
						alpha = 1.0f - ((float)(cg.time - cent->muzzleFlashTime)/SECONDS_1_F);	// what fraction of BARREL_SMOKE_TIME are we at
						alpha *= 0.25f;		// .25 max alpha
						CG_ParticleImpactSmokePuffExtended(
							cgs.media.smokeParticleShader, flash.origin, SECONDS_1, 8, 20, 30, alpha, 8.f);
					}
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer ) {
		if( (weaponNum == WP_STEN || weaponNum == WP_MP34 || weaponNum == WP_VENOM || (cg_drawMuzzleFlash.integer & 2 && weaponNum != WP_FLAMETHROWER) ) && !weapon->flashSmokeShader && ( cg.time - cent->muzzleFlashTime < SERVER_FRAMETIME_DELTA )) {
			//          shader file           |  origin     | life | vel | acc | roll | alpha | size
			CG_ParticleImpactSmokePuffExtended(
				cgs.media.smokeParticleShader, flash.origin,  SECOND_HALF,    8,    20,   30,   0.25f,  8.f );
		}
	...
	/* IRATA: old way ... called trap_R_AddRefEntityToScene( &flash ); twice a shot! 1st p AND !ps
	 * but we don't see the !ps stuff in 1st p view so I think this is useless here
	 *
	// jet Pilot - optionally disable muzzle flash per Ghost's request
	// draw muzzle flash for ALL weapons (not mounted tank & stationary MG) if enabled OR drawing a third person view
	if( flash.hModel && (cg_drawMuzzleFlash.integer & 1 || !ps)) {
		if(weaponNum != WP_FLAMETHROWER && ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME )) {
			CG_Printf("In CG_AddPlayerWeapon != WP_FLAMETHROWER %i %i %i\n",  cg.time, cent->muzzleFlashTime, cg.time - cent->muzzleFlashTime );
			trap_R_AddRefEntityToScene( &flash );
		}
	}
	*/
	...
	// jet Pilot - optionally disable muzzle flash per Ghost's request
	// draw muzzle flash for ALL weapons (not mounted tank & stationary MG) if enabled OR drawing a third person view
	if( flash.hModel && weaponNum != WP_FLAMETHROWER) {
		if (!ps && !isPlayer) {
			if( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME ) {
				trap_R_AddRefEntityToScene( &flash );
			}
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer ) {
		...
		if( firing && !(cent->currentState.eFlags & (EF_PRONE_MOVING | EF_DEAD)) ) {
			...
			// make a dlight for the flash - this is not the muzzle ...
...
called from CG_DrawActiveFrame
==============
*/
	...
	if( ( !cg_drawGun.integer ) ) {
		...
		if( cg.binocZoomTime ) {
			if( cg.binocZoomTime < 0 ) {
				...
				if( cg.binocZoomTime + SECOND_HALF < cg.time ) {
	...
	if( ps->eFlags & EF_MOUNTEDTANK ) {
		...
		if( cg.time - cg.predictedPlayerEntity.overheatTime < SECONDS_3 ) {
			if(!(rand()%3)) {
				...
				alpha *= 0.25f;		// .25 max alpha
				CG_ParticleImpactSmokePuffExtended( cgs.media.smokeParticleShader, cg.tankflashorg, SECONDS_1, 8, 20, 30, alpha, 8.f);
			}
		}

		if ( cg_drawMuzzleFlash.integer & 1 && cg.time - cg.predictedPlayerEntity.muzzleFlashTime < MUZZLE_FLASH_TIME) {
			refEntity_t		flash;
...
void CG_LastWeaponUsed_f( void ) {
	...
	if(cg.time - cg.weaponSelectTime < WEAPONCYCLEDELAY)
		return;	// force pause so holding it down won't go too fast

	if( IS_SET_WEAPON(cg.weaponSelect) ) {
		return;
	}
...
void CG_WeaponBank_f(void) {
	...
	if( ( cg.snap->ps.weaponstate == WEAPON_RAISING || cg.snap->ps.weaponstate == WEAPON_DROPPING ) &&
		( IS_RIFLENADE_WEAPON(curweap) || IS_RIFLENADE_WEAPON(num) || IS_SILENCED_PISTOL(curweap) || IS_SILENCED_PISTOL(num) ||
		  IS_MORTAR_WEAPON_SET(curweap) || IS_MORTAR_WEAPON_SET(num) ) )
		return;
...
void CG_DrawHitBoxOnHit( centity_t *cent ) {
	...
	if ( trace.fraction < 1.0f && (trace.contents & CONTENTS_BODY) ) {
		...
		if ( cg.nextSnap ) {
			...
			c->currentState.pos.trDuration = SECONDS_1 / 20; //sv_fps.integer;

			BG_EvaluateTrajectory( &c->currentState.pos, cg.snap->serverTime, origin1 , qfalse, c->currentState.effect2Time );
			BG_EvaluateTrajectory( &c->currentState.pos, cg.snap->serverTime + SECONDS_1 / 20, origin2 ,qfalse, c->currentState.effect2Time ); // replace 20 with sv_fps.integer if possible later
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, int surfFlags ) {	//	(SA) modified to send missilehitwall surface parameters
	...
	case WP_POISON_SYRINGE:
		if (!surfFlags) {
			...
			CG_AddBulletParticles( origin, dir, 50, SECOND_HALF, 2 + rand()%5, 1.5 );
		}
		else {
	...
	case WP_VENOM:
		...
		if (!clientNum) {
			...
			CG_WaterRipple(cgs.media.wakeMarkShaderAnim, origin, tv(0, 0, 1), 32, SECONDS_1);
...
// (SA) small modification.  only do this for non-rifles (so you can see your shots hitting when you're zooming with a rifle scope)
		if ( IS_SCOPED_WEAPON(weapon) || (Distance( cg.refdef_current->vieworg, origin ) < cg_markDistance.integer) ) {
			if(clientNum) {
...
// check for water/dirt spurt
		if (CG_PointContents( origin, 0 ) & CONTENTS_WATER) {
			...
			CG_WaterRipple(cgs.media.wakeMarkShaderAnim, trace.endpos, dir, 300, SECONDS_2);
			...
			if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
				CG_AddDirtBulletParticles( origin, dir, 400+random()*200, SECONDS_3, 10, 0.5, 400,256, 0.25, cgs.media.dirtParticle1Shader);
			}
 			for (i=0; i<3; ++i) {
				for (j=0;j<3;j++) {
				...
				CG_ParticleExplosion( "blacksmokeanim", sprOrg, sprVel, SECONDS_6 + random()*SECONDS_2, 40, 400+random()*200, qfalse ); // JPW NERVE was blacksmokeanimb
			}
			for (i=0; i<4; ++i) { // JPW random vector based on plane normal so explosions move away from walls/dirt/etc
				...
				CG_ParticleExplosion( "explode1", sprOrg, sprVel, SECONDS_1+rand()%1450, 40, 400+random()*200, (i == 0 ? qtrue : qfalse) );
			}
			CG_AddDebris( origin, dir, 400 + random()*200, rand()%SECONDS_2 + 1400, 12 + rand()%12 );
		}
		break;
	case WP_GRENADE_LAUNCHER:
	case WP_GRENADE_PINEAPPLE:
	...
	case WP_MORTAR2_SET:
		...
		else if( IS_MORTAR_WEAPON_SET(weapon) ) {
			sfx = sfx2 = 0;
		}
		else if( IS_HANDGRENADE(weapon) || IS_RIFLENADE_WEAPON(weapon) ) {
		...
		if (CG_PointContents( origin, 0 ) & CONTENTS_WATER) {
			...
			trap_CM_BoxTrace( &trace, tmpv,origin, NULL, NULL, 0, MASK_WATER );
			CG_WaterRipple(cgs.media.wakeMarkShaderAnim, trace.endpos, dir, 150, SECONDS_1);
			...
			trap_CM_BoxTrace(&trace,tmpv,tmpv2,NULL,NULL,0,MASK_SHOT);
			if (trace.surfaceFlags & SURF_GRASS || trace.surfaceFlags & SURF_GRAVEL) {
				CG_AddDirtBulletParticles( origin, dir, 400, SECONDS_2, 10, 0.5, 200,75, 0.25, cgs.media.dirtParticle1Shader);
			}
	...
	case WP_SMOKE_MARKER:
		...
		if (CG_PointContents( origin, 0 ) & CONTENTS_WATER) {
			...
			CG_AddDebris( origin, dir, 400+random()*200, rand()%SECONDS_2+SECONDS_1, 5 + rand()%5 );
		}
		break;
...
void CG_MissileHitWallSmall( int weapon, int clientNum, vec3_t origin, vec3_t dir ) {
	...
	static vec4_t	projection = {0, 0, -1.0f, 80.0f};
...
void CG_SpawnTracer( int sourceEnt, vec3_t pstart, vec3_t pend ) {
	...
	le->endTime = le->startTime + SECONDS_1_F * dist / cg_tracerSpeed.value;
...
// jpw
		...
		if (cg_gibs.integer && (lastBloodSpat > cg.time || lastBloodSpat < cg.time - SECOND_HALF) ) {
			...
			if ( CG_CalcMuzzlePoint( sourceEntityNum, start ) ) {
				...
				else if( lastBloodSpat < cg.time - SECONDS_1 ) {
