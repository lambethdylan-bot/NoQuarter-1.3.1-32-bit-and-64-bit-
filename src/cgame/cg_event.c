...
static void CG_Obituary( entityState_t *ent ) {
	...
	if( ca ) {
		...
		if ( (cg_insanity.integer & JP_INSANITY_KILLERCAM) &&
			cg.clientNum == cg.snap->ps.clientNum && target == cg.clientNum &&
			attacker >= 0 && attacker < MAX_CLIENTS && attacker != cg.clientNum && !(cg.snap->ps.pm_flags & PMF_FOLLOW) )
		{
			// set the killer..
	...
	case EV_GENERAL_SOUND:
		DEBUGNAME("EV_GENERAL_SOUND");
		{
			...
			if ( sound ) {
				...
				if ( es->eventParm >= GAMESOUND_MAX && (es->eventParm - GAMESOUND_MAX) < MAX_SOUNDS ) {
					const char			*s;
