...
qboolean CG_EntOnFire( centity_t *cent ) {
	if ( cent->currentState.number == cg.snap->ps.clientNum && cent->currentState.eType != ET_CORPSE ) {
		...
		return	( cg.snap->ps.onFireStart
			...
			&& ((cg.snap->ps.onFireStart+SECONDS_2) > cg.time) );
	}
	else {
...
void CG_NewClientInfo( int clientNum ) {
	...
	while(1) {
		...
		case TOK_ch:
			if(cs_value[0]) {
				int chIdx = atoi(cs_value);
				if (chIdx >= 0 && chIdx < MAX_CHARACTERS) {
					newInfo.character = cgs.gameCharacters[chIdx];
				}
			}
			break;
		case TOK_s:
		...
		case TOK_xp:
			...
			if( *buffer ) {
				int i;
				char *skill = strtok( buffer , " ");

				for (i = 0; i < SK_NUM_SKILLS && skill; i++) {
	...
	if( ISVALIDCLIENTNUM(cg.clientNum) && clientNum == cg.clientNum ) {
		int i;
		int j;
		int oldBits;

		if( newInfo.team != cgs.clientinfo[ cg.clientNum ].team ) {
			if( cgs.autoFireteamCreateEndTime != cg.time + SECONDS_20) {
				cgs.autoFireteamCreateEndTime = 0;
			}
			if( cgs.autoFireteamJoinEndTime != cg.time + SECONDS_20) {
				cgs.autoFireteamJoinEndTime = 0;
			}
		}

		if( newInfo.rank > cgs.clientinfo[ cg.clientNum ].rank && newInfo.rank < NUM_EXPERIENCE_LEVELS && newInfo.rank >= 0 ) {
		...
		if ( cg.clientNum == clientNum ) {
			if ( cgs.levelStartTime > 0 && cg.time < cgs.levelStartTime + SECONDS_10 ) {
...
#define ANIM_SPEEDMAX_HIGH	20
	...
	if ( anim->moveSpeed && lf->oldFrameSnapshotTime ) {
		...
		if ( cg.latestSnapshotTime != lf->oldFrameSnapshotTime && cg.nextSnap ) {
			...
			if ( cent->currentState.number == cg.snap->ps.clientNum ) {
				...
				moveSpeed = Distance( cent->lerpOrigin, lf->oldFramePos ) / ( (float) (cg.time - lf->oldFrameTime) / SECONDS_1_F );
			}
			else {
				if ( isLadderAnim ) {
					// only use Z axis for speed
	...
	if ( lf->frameTime > cg.time + SECONDS_5 ) {
		lf->frameTime = cg.time;
	}
...
static void CG_BreathPuffs( centity_t *cent, refEntity_t *head ) {
	...
	CG_SmokePuff( morg, forward, 4, 1, 1, 1, 0.5f, SECONDS_2, cg.time, cg.time + 400, 0, cgs.media.shotgunSmokePuffShader );

	ci->breathPuffTime = cg.time + SECONDS_3 + random()*SECONDS_1;
}
...
qboolean CG_WorldCoordToScreenCoordFloat(vec3_t point, float *x, float *y) {
	...
	if(z < 0.1f) return qfalse;
	px *= z;
	py *= z;
	if (px == 0 || py == 0) return qfalse;
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if ( cent->currentState.eFlags & EF_CONNECTION /*&& sameTeam*/  ) // IRATA: shown for both teams now ... 
	...
	// WIP/TODO/FIXME: Protect disguised enemy is easy to detect ...
	// full skilled covert ops also have a fireteam icon if they are in a fireteam :o)
	/*
	if ( disguise ) { // checks team, disguise and skill of enemy
		fireteamData_t* ft;
		if (ft == CG_IsOnFireteam( X )) {
	...
	*/
	...
	// core: draw a buy-icon when the player is buying (shrubbot !buy command)
	if( cent->currentState.eFlags & EF_BUYING )
		CG_PlayerFloatSprite( cent, cgs.media.buyIcon, height+8, numIcons++ );
}
...
//	cent->pe.bodyRefEnt = body;
	...
	// DEBUG
	if (cg_drawHitbox.integer ) { // IRATA: let's check if cg_drawHitbox is set first (I do assume it's not in most cases)
		// position marker?
...
//	head.renderfx = renderfx;
	...
	if( cent->currentState.eFlags & EF_FIRING ) {
		...
		if( cent->pe.weaponFireTime && cent->pe.weaponFireTime > SECOND_HALF ) {
...
// Meyer - Temp change
	...
	if ( fullDraw && cent->currentState.eType != ET_CORPSE && !(cent->currentState.eFlags & EF_PLAYDEAD) && lodBias <= 0 ) {
		float dist = VectorDistance(cent->currentState.pos.trBase, cg.refdef_current->vieworg);

		if ( dist < 1024 ) {
			hudHeadAnimNumber_t anim = cent->pe.idleAnim;
			animateHead = qtrue;

			if ( cent->pe.weaponFireTime > SECOND_HALF && !(cent->currentState.eFlags & EF_DEAD) ) {
				anim = HD_ATTACK;
			}
			else if ( cg.time - cent->pe.lastFiredWeaponTime < SECOND_HALF && !(cent->currentState.eFlags & EF_DEAD) ) {
				anim = HD_ATTACK_END;
			}
				...
				cent->pe.nextIdleTime = cg.time + SECONDS_7 + rand() % SECONDS_1;
...
void CG_ResetPlayerEntity( centity_t *cent ) {
	...
	if (!(cent->currentState.eFlags & EF_DEAD)) {
		...
		memset( &cent->pe.torso, 0, sizeof( cent->pe.torso ) ); // IRATA: it's torso not legs
