...
#include "cg_local.h"
 ...
 // MODERN INTERMISSION TABS
  // 0 = Scoreboard, 1 = Awards, 2 = Stats, 3 = Vote Now
int intermissionTab = 0;
...
qboolean CG_DrawScoreboard( void ) {
	...
	//Added to fit modern style intermission UI

	if (cgs.gametype == GT_WOLF_STOPWATCH && (cg.snap->ps.pm_type == PM_INTERMISSION)) {
		y = WM_DrawInfoLine(x, 155, fade);

		// LEGACY STYLE: Side-by-side, capped at 15 players max to close the gap
		WM_TeamScoreboard(x, y, TEAM_AXIS, fade, 15, 15);
		x = x_right;
		WM_TeamScoreboard(x, y, TEAM_ALLIES, fade, 15, 15);
	}
	else {
		if (cg.snap->ps.pm_type == PM_INTERMISSION) {
			// LEGACY STYLE: Side-by-side, capped at 15 players max to close the gap
			...
			// STANDARD IN-GAME TAB MENU (Keep Side-by-Side, uncapped)
