...
int Text_Height( const char *text, float scale, int limit ) {
	//fontInfo_t *font = &uiInfo.uiDC.Assets.fonts[uiInfo.activeFont];
	return Text_Height_Ext( text, scale, limit, &uiInfo.uiDC.Assets.fonts[uiInfo.activeFont] );
}
...
void Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style) {
	...
	// core: this is needed for the scrolling credits to work properly..
	if ( y >= 480 ) return;

	Text_Paint_Ext( x, y, scale, scale, color, text, adjust, limit, style, &uiInfo.uiDC.Assets.fonts[uiInfo.activeFont] );
}
...
int Count_Text_AutoWrap_Paint(float x, float y, int width, float scale, vec4_t color, const char *text, float adjust, int style) {
	const char *ret = text, *end = text + strlen(text);
	int i = 0;
...
void _UI_Refresh( int realtime ) {
	...
	if ( index > UI_FPS_FRAMES ) {
		...
		uiInfo.uiDC.FPS = SECONDS_1 * UI_FPS_FRAMES / total;
	}
...
void UI_DrawMapPreview(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	...
	if( game == GT_WOLF_CAMPAIGN ) {
		...
		// jaquboss, draw levelshot instead of campaign location while non campaign

		if (uiInfo.mapList[map].mapLoadName) {
			// IRATA: check if file is in path before registering ! (tga, TGA, jpg, JPG) ...
			qhandle_t preview = trap_R_RegisterShaderNoMip( va("levelshots/%s",uiInfo.mapList[map].mapLoadName ));
...
void UI_DrawCampaignDescription( rectDef_t *rect, float scale, vec4_t color, float text_x, float text_y, int textStyle, int align, qboolean net ) {
	...
	char buff[1024];
	int height, len=0, textWidth, newLine=0, newLineWidth=0;
	float y=0;
	rectDef_t textRect;
...
void UI_DrawGametypeDescription( rectDef_t *rect, float scale, vec4_t color, float text_x, float text_y, int textStyle, int align, qboolean net ) {
	...
	char buff[1024];
	int height, len=0, textWidth, newLine=0, newLineWidth = 0, i;
	float y = 0;
	rectDef_t textRect;
	int game = (net) ? ui_netGameType.integer : ui_netGameType.integer;
...
static void UI_DrawCampaignMapDescription(rectDef_t *rect, float scale, vec4_t color, float text_x, float text_y, int textStyle, int align, qboolean net, int number) {
	...
	const char *p, *textPtr, *newLinePtr=NULL;
	char buff[1024];
	int height, len=0, textWidth=0, newLine=0, newLineWidth=0;
	float y;
	rectDef_t textRect;
...
// jaquboss : this is somewhat Q3ish and a bit RTCWish code, needs to be redone
...
static void UI_DrawSelectedPlayer(rectDef_t *rect, float scale, vec4_t color, int textStyle) {
	if (uiInfo.uiDC.realTime > uiInfo.playerRefresh) {
		uiInfo.playerRefresh = uiInfo.uiDC.realTime + SECONDS_3;
		UI_BuildPlayerList();
	}
...
static void UI_ParseGLConfig( void ) {
	char *eptr = uiInfo.uiDC.glconfig.extensions_string;
...
static void UI_DelSavegame() {
	...
	int ret = trap_FS_Delete(va("save/%s.svg", uiInfo.savegameList[uiInfo.savegameIndex].name));
...
//----(SA)	end
			...
			char *s = UI_Cvar_VariableString("com_errorDiagnoseIP");
			menuDef_t *menu = Menus_FindByName("ingame_options");
		...
		else if (Q_stricmp(name, "votePoll") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				char s[MAX_EDIT_LINE];
				trap_Cvar_VariableStringBuffer( "ui_poll", s, sizeof(s) );
		...
		// DHM - Nerve
			...
			// get old mode
			int		oldMode  = trap_Cvar_VariableValue( "r_oldMode" );
			...
			float ui_r_customWidth = trap_Cvar_VariableValue( "r_customWidth" );
			float ui_r_customHeight = trap_Cvar_VariableValue( "r_customHeight" );
			//---
			...
			//--- core: widescreen support..
			trap_Cvar_Set( "ui_r_customWidth", va("%f",ui_r_customWidth) );
			trap_Cvar_Set( "ui_r_customHeight", va("%f",ui_r_customHeight) );
			//---
			...
			//--- core: widescreen support..
			trap_Cvar_Set( "ui_r_customWidth", "" );
			trap_Cvar_Set( "ui_r_customHeight", "" );
			//---
			...
			//--- core: widescreen support..
			float ui_r_customWidth = trap_Cvar_VariableValue( "ui_r_customWidth" );
			float ui_r_customHeight = trap_Cvar_VariableValue( "ui_r_customHeight" );
			float fovx = 90.0f;
			//---
			...
			switch (ui_r_mode) {
				case -1:
					fovx = (ui_r_customWidth/ui_r_customHeight) * 73.74f;	// default fovy is 73.74
					DC->executeText(EXEC_APPEND, va("cg_fov %3.2f ; cg_fovy 73.74 ; ",fovx));
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", va("%f",ui_r_customWidth) );
					trap_Cvar_Set( "r_customHeight", va("%f",ui_r_customHeight) );
					break;
				case 11:	// "856*480 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 106.26 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", va("%i",ui_r_mode) );
					trap_Cvar_Set( "r_customWidth", "856" );
					trap_Cvar_Set( "r_customHeight", "480" );
					break;
				case 100:	// "1920*1080 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 106.26 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1920" );
					trap_Cvar_Set( "r_customHeight", "1080" );
					break;
				case 101:	// "1366*768 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 106.26 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1366" );
					trap_Cvar_Set( "r_customHeight", "768" );
					break;
				case 102:	// "1280*720 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 106.26 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1280" );
					trap_Cvar_Set( "r_customHeight", "720" );
					break;
				case 103:	// "1920*1200 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 100.39 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1920" );
					trap_Cvar_Set( "r_customHeight", "1200" );
					break;
				case 104:	// "1680*1050 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 100.39 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1680" );
					trap_Cvar_Set( "r_customHeight", "1050" );
					break;
				case 105:	// "1440*900 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 100.39 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1440" );
					trap_Cvar_Set( "r_customHeight", "900" );
					break;
				case 106:	// "1280*800 Wide Screen"
					DC->executeText(EXEC_APPEND, "cg_fov 100.39 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", "-1" );
					trap_Cvar_Set( "r_customWidth", "1280" );
					trap_Cvar_Set( "r_customHeight", "800" );
					break;
				default:
					DC->executeText(EXEC_APPEND, "cg_fov 90.00 ; cg_fovy 73.74 ; ");
					trap_Cvar_Set( "r_mode", va("%i",ui_r_mode) );
					trap_Cvar_Set( "r_customWidth", "" );
					trap_Cvar_Set( "r_customHeight", "" );
					break;
			}
			//---
...
*/
static void UI_BuildServerDisplayList(qboolean force) {
	int i, count, clients, maxClients, ping, game = 0, len, friendlyFire, maxlives, punkbuster, antilag, password, weaponrestricted, balancedteams;
	...
	for (i = 0; i < count; i++) {
		...
		if (ping >/*=*/ 0 || ui_netSource.integer == AS_FAVORITES) {
			...
			if ( ui_browserShowFriendlyFire.integer ) {
				...
				if( ( friendlyFire & 1 && ui_browserShowFriendlyFire.integer == 2 ) ||
					( !(friendlyFire & 1) && ui_browserShowFriendlyFire.integer == 1 ) ) {
					trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
					continue;
				}
			}
			...
			trap_Cvar_Update( &ui_browserNQonly );

			{
				const char	*gamename = Info_ValueForKey( info, "game" );

				if( (  !Q_stricmp( gamename, "nq") && ui_browserNQonly.integer == 2 ) ||
					(  Q_stricmp( gamename, "nq") && ui_browserNQonly.integer == 1 ) ) {
					trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
					continue;
				}
			}
...
static int UI_GetServerStatusInfo( const char *serverAddress, serverStatusInfo_t *info ) {
	...
	if ( trap_LAN_ServerStatus( serverAddress, info->text, sizeof(info->text)) ) {
		...
		int i, len;
		// we use the URL buttons in several menus
		menuDef_t *menu = Menus_FindByName("serverinfo_popmenu");
		menuDef_t *menu2 = Menus_FindByName("popupError");
...
static const char *UI_SelectedMap(qboolean singlePlayer, int index, int *actual) {
	int i, c = 0, game = singlePlayer ? uiInfo.gameTypes[ui_gameType.integer].gtEnum : ui_netGameType.integer;
...
static void UI_UpdatePendingPings() {
	...
	uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + SECONDS_1;
}
	...
	*numhandles = 0;
	...
	if (feederID == FEEDER_HEADS) {
		...
		if( index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
			int ping, game, antilag, needpass, friendlyfire, maxlives, punkbuster, weaponrestrictions, balancedteams, serverload, noquarter;
			if (lastColumn != column || lastTime > uiInfo.uiDC.realTime + SECONDS_5) {
				...
				case SORT_FILTERS:
					if( ping <= 0 ) {
						...
						*numhandles = 8;
						...
						antilag = atoi( Info_ValueForKey( info, "g_antilag" ) );
						balancedteams = atoi( Info_ValueForKey( info, "balancedteams" ) );
						noquarter = (!Q_stricmp(Info_ValueForKey( info, "game" ), "nq") ? 1 : 0);
						...
						if( noquarter ) handles[7] = uiInfo.nqFilter; else handles[7] = -1;
...
void UI_FeederSelection(float feederID, int index) {
	...
	if (feederID == FEEDER_HEADS) {
		...
		int actual;
		int game = feederID == FEEDER_MAPS ? uiInfo.gameTypes[ui_gameType.integer].gtEnum : ui_netGameType.integer;
...
void _UI_Init( qboolean inGameLoad ) {
	...
	uiInfo.nqFilter = trap_R_RegisterShaderNoMip( "ui/assets/filter_nq.tga" );

	uiInfo.campaignMap = trap_R_RegisterShaderNoMip( "gfx/loading/camp_map.tga" );

	// core: images used for menu/button effects..
	imgBlingbling = trap_R_RegisterShaderNoMip("ui/assets/blingbling.tga");
	imgFadeToBlack = trap_R_RegisterShaderNoMip("ui/assets/fadetoblack.tga");
	imgFadeCircle = trap_R_RegisterShaderNoMip("ui/assets/fadecircle.tga");
...
#ifdef HW_BAN
	trap_AddCommand( "hwguid" );
	trap_AddCommand( "hwinfo" );
#endif // HW_BAN
}
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	if (Menu_Count() > 0) {
		...
		case UIMENU_MAIN:
			...
			trap_S_FadeAllSound( 1.0f, SECONDS_1, qfalse );	// make sure sound fades up
...
// jpw
	...
	{ &ui_browserNQonly, "ui_browserNQonly", "1", CVAR_ARCHIVE },
	...
	// { NULL, "cg_drawBuddies", "1", CVAR_ARCHIVE }, // IRATA: not used in engine or mod code
	...
	//--- core: widescreen support..
	{ NULL, "ui_r_customWidth", "", CVAR_ARCHIVE },
	{ NULL, "ui_r_customHeight", "", CVAR_ARCHIVE },
	//---
	...
	{ NULL,	"cg_drawHUDHead",				 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_drawObjIcons",				 "0",		 CVAR_ARCHIVE },
	{ NULL,	"cg_markDistance",				 "384",		 CVAR_ARCHIVE },
	{ NULL,	"hud_powerupSize",				 "18",		 CVAR_ARCHIVE },
	{ NULL,	"hud_skillBarX",				 "44",		 CVAR_ARCHIVE },
	{ NULL,	"hud_skillBarY",				 "388",		 CVAR_ARCHIVE },
	{ NULL,	"hud_skillBarAlpha",			 "1.0",		 CVAR_ARCHIVE },
	{ NULL,	"hud_rankX",					 "112",		 CVAR_ARCHIVE },
	{ NULL,	"hud_rankY",					 "408",		 CVAR_ARCHIVE },
	{ NULL,	"hud_rankAlpha",				 "1.0",		 CVAR_ARCHIVE },
	{ NULL,	"hud_drawPowerups",				 "1",		 CVAR_ARCHIVE },
	{ NULL,	"hud_drawAltHUD",				 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_drawMuzzleFlash",			 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_drawTracers",				 "1",		 CVAR_ARCHIVE },
	...
	{ NULL,	"cg_fixedFTeamSize",			 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_weaponBounceSound",			 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_altHQVoice",				 "0",		 CVAR_ARCHIVE },
	{ NULL,	"cg_rtcwShake",					 "0",		 CVAR_ARCHIVE },
	{ NULL,	"cg_weapAltReloads",			 "1",		 CVAR_ARCHIVE },
	{ NULL,	"cg_drawAuraIcons",				 "1",		 CVAR_ARCHIVE },
...
static void UI_StartServerRefresh( qboolean full ) {
	...
	uiInfo.serverStatus.refreshActive = qtrue;
	uiInfo.serverStatus.nextDisplayRefresh = uiInfo.uiDC.realTime + SECONDS_1;
	// clear number of displayed servers
	uiInfo.serverStatus.numDisplayServers = 0;
	uiInfo.serverStatus.numPlayersOnServers = 0;
	// mark all servers as visible so we store ping updates for them
	...
	uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + SECONDS_5;
