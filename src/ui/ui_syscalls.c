 ...
 * name:	ui_syscalls.c
 *
 * desc:	this file is only included when building a dll
...
float trap_Cvar_VariableValue( const char *var_name ) {
	int temp = syscall( UI_CVAR_VARIABLEVALUE, var_name );
...
qhandle_t trap_R_RegisterModel( const char *name ) {
	...
	handle = syscall( UI_R_REGISTERMODEL, name );
	if (!*name) {
		Com_DPrintf("^1Warning: Null ui model filename\n");
	}
	if (handle == 0) {
		Com_DPrintf("^1Warning: Failed to load ui model: %s\n", name);
	}
	return handle;
}
...
qhandle_t trap_R_RegisterSkin( const char *name ) {
	...
	handle = syscall( UI_R_REGISTERSKIN, name );
	if (!*name) {
		Com_DPrintf("^1Warning: Null ui skin filename\n");
	}
	if (handle == 0) {
		Com_DPrintf("^1Warning: Failed to load ui skin: %s\n", name);
	}
	return handle;
}
...
qhandle_t trap_R_RegisterShaderNoMip( const char *name ) {
	...
	handle = syscall( UI_R_REGISTERSHADERNOMIP, name );
	if (!*name) {
		Com_DPrintf("^1Warning: Null ui shader no mip filename\n");
	}
	if (handle == 0) {
		Com_DPrintf("^1Warning: Failed to load ui shader no mip: %s\n", name);
	}
