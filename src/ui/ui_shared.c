 ...
 * name:	ui_shared.c
 *
 * desc:	string allocation/managment
 *
 * NQQS:
 *
 */
...
typedef struct {				// backup original y-coordinates,
	float t;					// so the items can be reset to
	float w;					// their start-positions,
} scrollYpos_t;					// in case the menu must be drawn again.

typedef struct {
	menuDef_t *menu;
	scrollYpos_t scrollYpos[MAX_MENUITEMS];
} scrollYmenu_t;
scrollYmenu_t scrollYmenu;

void RestoreYcoords( void ) {
	menuDef_t *menu;
	int i;

	scrolltime = 0; // reset time for next run..

	if ( !scrollYmenu.menu ) {
		return;
	}

	menu = scrollYmenu.menu;
	for ( i=0; i<menu->itemCount; ++i ) {
		if ( !(menu->items[i]->window.background || menu->items[i]->asset || menu->items[i]->text) )
			continue;
		if ( menu->items[i]->text )
			menu->items[i]->textRect.y = scrollYmenu.scrollYpos[i].t;
		menu->items[i]->window.rect.y = scrollYmenu.scrollYpos[i].w;
	}
	scrollYmenu.menu = NULL;
}

void BackupYcoords( menuDef_t *menu ) {
	int i;
	...
	scrollYmenu.menu = menu;
	for ( i=0; i<menu->itemCount; ++i ) {
		if ( !(menu->items[i]->window.background || menu->items[i]->asset || menu->items[i]->text) )
			continue;
		if ( menu->items[i]->text )
			scrollYmenu.scrollYpos[i].t = menu->items[i]->textRect.y;
		scrollYmenu.scrollYpos[i].w = menu->items[i]->window.rect.y;
	}
}
//---
...
#else
#define MEM_POOL_SIZE  4096 * 1024	// core: was 1536 (about 112 menus), Arnout: was 1024
#endif
...
static void Menu_RunCloseScript(menuDef_t *menu) {
	...
	RestoreYcoords(); // core: scrolling window content..
}
...
// core: toggle cvar bits from menu script..
// This is for integer cvars using bitflags.
	...
	if( Int_Parse(args, &useprofile) ) {
		...
		if( useprofile && cl_profileStr[0] ) {
			...
			Com_sprintf(bakPath, sizeof(bakPath),
				"profiles/%s/backup_%s", cl_profileStr, CONFIG_NAME);
...
commandDef_t commandList[] =
{
	...
	//--- core: toggle bits for integer cvar (for using bitflags)
...
int Item_ListBox_OverLB(itemDef_t *item, float x, float y) {
	...
	count = DC->feederCount(item->special); // IRATA: obsolete?
...
void Item_Text_Paint(itemDef_t *item) {
	...
	if (item->text == NULL) {
		if (item->cvar == NULL) {
			...
			if( item->window.flags & WINDOW_TEXTASINT ) {
				COM_StripExtensionSafe(text, text, sizeof(text));
	...
	if( item->type == ITEM_TYPE_TIMEOUT_COUNTER && menu != NULL && menu->openTime > 0 ) {
		...
		int seconds = (menu->openTime + menu->timeout - DC->realTime + 999) / SECONDS_1;
	...
	// core: blingbling overlay..
	if( item->type == ITEM_TYPE_BUTTON ){
		// shine effect
		if ( imgFadeCircle && (item->window.flags & (WINDOW_HASFOCUS|WINDOW_MOUSEOVER)) == (WINDOW_HASFOCUS|WINDOW_MOUSEOVER) ) {
			float x = item->window.rect.x - 10.0f;
			float y = item->window.rect.y - 25.0f;
			float w = item->window.rect.w + 20.0f;
			float h = item->window.rect.h + 50.0f;
			AdjustFrom640(&x,&y,&w,&h);
			DC->drawStretchPic( x, y, w, h, 0,0,1,1, imgFadeCircle );
		}
		// glossy effect
		if ( imgBlingbling ) {
			float x = item->window.rect.x;
			float y = item->window.rect.y;
			float w = item->window.rect.w;
			float h = item->window.rect.h;
			AdjustFrom640(&x,&y,&w,&h);
			DC->drawStretchPic( x, y, w, h, 0,0,1,1, imgBlingbling );
		}
	}
...
void Item_CheckBox_Paint( itemDef_t *item ) {
	...
	// core: bitflag value?
	if ( item->bitflag ) {
		value = ( (int)(DC->getCVarValue(item->cvar)) & item->bitflag );
	} else {
...
static bind_t g_bindings[] =
{
	...
	{ "dropweapon",			'j',			-1,		'j',			-1,		-1,	-1 },
	{ "dropclip",			'n',			-1,		'n',			-1,		-1,	-1 },
...
char* BindingFromName(const char *cvar) {
	...
	if( b1 != -1 ) {
		...
		if(b2 != -1) {
			DC->keynumToStringBuf( b2, g_nameBind2, sizeof(g_nameBind1) );
		...
		// jP - kw - show bind names instead of (???)
...
void Item_Bind_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_HASFOCUS && item->window.flags & WINDOW_FOCUSPULSE) {
		if (g_bindItem == item) {
			lowLight[0] = 0.3f * 1.0f;
			lowLight[1] = 0.3f * 0.0f;
			lowLight[2] = 0.3f * 0.0f;
			lowLight[3] = 0.3f * 1.0f;
		} else {
			lowLight[0] = 0.3f * parent->focusColor[0];
			lowLight[1] = 0.3f * parent->focusColor[1];
			lowLight[2] = 0.3f * parent->focusColor[2];
			lowLight[3] = 0.3f * parent->focusColor[3];
		}
		LerpColor(parent->focusColor,lowLight,newColor,0.5+0.5*sin(DC->realTime / PULSE_DIVISOR));
	} else {
		if( g_bindItem == item ) {
...
#define NEWWAY
	...
	if(modelPtr->frameTime)	// don't advance on the first frame
		modelPtr->backlerp+=( ((DC->realTime - modelPtr->frameTime)/SECONDS_1_F) * (float)modelPtr->fps );
...
void Item_Init(itemDef_t *item) {
	...
	// core: bitflags for integer cvars
	item->bitflag = 0;
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
	...
	// core: scrolling window-content..
	if ( menu->window.flags & WINDOW_SCROLLING ) {
		int deltatime;
		float deltamove;

		// first time initialization..
		if ( scrolltime==0 ) {
			// backup original Y-coordinates..
			BackupYcoords( menu );
			// find the bottom items' y-coordinate,..
			itemy = 480;
			for ( i=0; i<menu->itemCount; ++i ) {
				if ( !(menu->items[i]->window.background || menu->items[i]->asset || menu->items[i]->text) )
					continue;
				if ( menu->items[i]->text )
					if ( menu->items[i]->textRect.y > itemy )
						itemy = menu->items[i]->textRect.y;
				if ( menu->items[i]->window.rect.y > itemy )
					itemy = menu->items[i]->window.rect.y;
			}
			if ( itemy > 480 )
				itemy += 40;	// spacing..

			scrolltime = DC->realTime;
		}

		deltatime = DC->realTime - scrolltime;
		scrolltime = DC->realTime;
		deltamove = (float)(scrolling * deltatime / 100.0);
		// scrollspeed on mouse Y position..
		deltamove *= (float)((float)(DC->cursory) / (float)480.0);
		// scroll..
		for (i = 0; i < menu->itemCount; ++i) {
			if ( !(menu->items[i]->window.background || menu->items[i]->asset || menu->items[i]->text) )
				continue;
			// texts
			if ( menu->items[i]->text ) {
				menu->items[i]->textRect.y -= deltamove;
				if ( menu->items[i]->textRect.y < -100 )
					menu->items[i]->textRect.y = itemy;
			}
			// all items' rect
			menu->items[i]->window.rect.y -= deltamove;
			if ( menu->items[i]->window.rect.y < -100 )
				menu->items[i]->window.rect.y = itemy;
		}
	}
	...
	// core: scrolling window-content..
	if ( menu->window.flags & WINDOW_SCROLLING ) {
		float h120 = 120 * DC->yscale;
		float xx = menu->window.rect.x * DC->xscale;
		float ww = menu->window.rect.w * DC->xscale;
		DC->drawStretchPic( xx-16, menu->window.rect.y*DC->yscale-8, ww+32, h120+8, 0,0,1,1, imgFadeToBlack );
		DC->drawStretchPic( xx-16, menu->window.rect.h*DC->yscale-h120, ww+32, h120+8, 0,1,1,0, imgFadeToBlack );
	}
}
...
keywordHash_t itemParseKeywords[] =
{
	...
	// core: a bitflag for use with bits in integer cvars
...
qboolean MenuParse_style( itemDef_t *item, int handle ) {
	...
	if (i >= 128) {
		menu->window.flags |= WINDOW_SCROLLING;
		i -= 128;
	}
	menu->window.style = i;
	return qtrue;
...
void BG_PanelButton_RenderEdit( panel_button_t* button ) {
	...
	if( useCvar ) {
		...
		if( BG_PanelButtons_GetFocusButton() == button && ((DC->realTime / SECONDS_1) % 2)) {
...
// core: adjusting panel coordinates so it is horizontally centered..
